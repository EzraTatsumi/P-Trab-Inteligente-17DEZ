...
### 2. Atualizando `src/integrations/supabase/api.ts`

Vou garantir que `fetchArpsByUasg` mapeie o `nomeUnidadeGerenciadora` para `omNome` no `ArpItemResult`.

<dyad-write path="src/integrations/supabase/api.ts" description="Mapeando nomeUnidadeGerenciadora para omNome no ArpItemResult.">
    ...
    try {
        ...
        const results: ArpItemResult[] = responseData.map((item: ArpRawResult) => {
            ...
            return {
                ...
                uasg: uasgStr,
                omNome: item.nomeUnidadeGerenciadora || `UASG ${uasgStr}`, // Usar o nome da OM da API
