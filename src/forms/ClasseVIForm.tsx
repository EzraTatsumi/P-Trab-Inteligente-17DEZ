...
const ClasseVIFormSchema = z.object({
...
type ClasseVIFormValues = z.infer<typeof ClasseVIFormSchema>;

interface ClasseVIFormProps {
  initialData?: ClasseVIRegistro;
  onSuccess: () => void;
}
...
export function ClasseVIForm({ initialData, onSuccess }: ClasseVIFormProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["classe_vi_registros", pTrab.id] });
      toast.success(initialData ? "Registro atualizado com sucesso!" : "Registro criado com sucesso!");
      onSuccess();
    },
    onError: (error) => {
      console.error("Erro ao salvar registro de Classe VI:", error);
      toast.error("Erro ao salvar registro. Tente novamente.");
    },
  });

  const onSubmit = (values: ClasseVIFormValues) => {
