...
import {
  ...
  ClasseVIUpdate,
  ClasseVI,
} from "@/integrations/supabase/classeVI";
...
const formSchema = z.object({
  ...
  om_id: z.string().optional().nullable(), // ID da OM selecionada (não persistido na tabela classe_vi_registros)
});

type ClasseVIFormValues = z.infer<typeof formSchema>;

// --- Component ---

interface ClasseVIFormProps {
  pTrabId: string;
  initialData?: ClasseVI;
  onSuccess?: () => void;
}

export function ClasseVIForm({ pTrabId, initialData, onSuccess }: ClasseVIFormProps) {
  ...
  const onSubmit = async (values: ClasseVIFormValues) => {
    // ... (submission logic)
  };
