...
import {
  ...
  ClasseVIIUpdate,
  ClasseVII,
} from "@/integrations/supabase/classeVII";
...
const formSchema = z.object({
  ...
  om_id: z.string().optional().nullable(), // ID da OM selecionada (não persistido na tabela classe_vii_registros)
});

type ClasseVIIFormValues = z.infer<typeof formSchema>;

// --- Component ---

interface ClasseVIIFormProps {
  pTrabId: string;
  initialData?: ClasseVII;
  onSuccess?: () => void;
}

export function ClasseVIIForm({ pTrabId, initialData, onSuccess }: ClasseVIIFormProps) {
  ...
  const onSubmit = async (values: ClasseVIIFormValues) => {
    // ... (submission logic)
  };
