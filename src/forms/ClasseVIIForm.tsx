...
// Tipagem do registro de Classe VII
export interface ClasseVIIRegistro {
  id: string;
  p_trab_id: string;
...
const ClasseVIIFormSchema = z.object({
...
type ClasseVIIFormValues = z.infer<typeof ClasseVIIFormSchema>;

interface ClasseVIIFormProps {
  initialData?: ClasseVIIRegistro;
  onSuccess: () => void;
}

export function ClasseVIIForm({ initialData, onSuccess }: ClasseVIIFormProps) {
  const queryClient = useQueryClient();
  const { pTrab } = usePTrabContext();

  const form = useForm<ClasseVIIFormValues>({
    resolver: zodResolver(ClasseVIIFormSchema),
    defaultValues: {
  ...
  const mutation = useMutation({
    mutationFn: async (data: ClasseVIIFormValues) => {
      const payload = {
        ...data,
        p_trab_id: pTrab.id,
        valor_total: 0,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["classe_vii_registros", pTrab.id] });
      toast.success(initialData ? "Registro atualizado com sucesso!" : "Registro criado com sucesso!");
      onSuccess();
    },
    onError: (error) => {
      console.error("Erro ao salvar registro de Classe VII:", error);
      toast.error("Erro ao salvar registro. Tente novamente.");
    },
  });

  const onSubmit = (values: ClasseVIIFormValues) => {
