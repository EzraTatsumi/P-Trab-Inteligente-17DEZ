...
// Tipagem do registro de Classe V
export interface ClasseVRegistro {
  id: string;
  p_trab_id: string;
...
const ClasseVFormSchema = z.object({
...
type ClasseVFormValues = z.infer<typeof ClasseVFormSchema>;

interface ClasseVFormProps {
  initialData?: ClasseVRegistro;
  onSuccess: () => void;
}

export function ClasseVForm({ initialData, onSuccess }: ClasseVFormProps) {
  const queryClient = useQueryClient();
  const { pTrab } = usePTrabContext();

  const form = useForm<ClasseVFormValues>({
    resolver: zodResolver(ClasseVFormSchema),
    defaultValues: {
  ...
  const mutation = useMutation({
    mutationFn: async (data: ClasseVFormValues) => {
      const payload = {
        ...data,
        p_trab_id: pTrab.id,
        valor_total: 0,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["classe_v_registros", pTrab.id] });
      toast.success(initialData ? "Registro atualizado com sucesso!" : "Registro criado com sucesso!");
      onSuccess();
    },
    onError: (error) => {
      console.error("Erro ao salvar registro de Classe V:", error);
      toast.error("Erro ao salvar registro. Tente novamente.");
    },
  });

  const onSubmit = (values: ClasseVFormValues) => {
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    <SelectItem value="Munição">Munição</SelectItem>
