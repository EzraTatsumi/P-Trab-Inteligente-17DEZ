...
interface LocacaoCatalogDialogProps {
    open: boolean;
...
const fetchCatalogItems = async (): Promise<CatalogoSubitem[]> => {
    const { data, error } = await supabase
        .from('catalogo_locacoes_nd' as any)
        .select('*')
        .eq('ativo', true)
        .order('nr_subitem', { ascending: true });

    if (error) {
        console.error("Erro ao buscar catálogo de locações:", error);
...
export default LocacaoCatalogDialog;