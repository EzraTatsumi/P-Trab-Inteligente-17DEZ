...
// Função auxiliar para normalizar strings (remover espaços e converter para maiúsculas)
...
const getArticleForOM = (omName: string): 'DO' | 'DA' => {
  const normalizedOmName = normalizeString(omName);
...
const PTrabOperacionalReport: React.FC<PTrabOperacionalReportProps> = ({
  ...
  const { registrosAgrupadosPorOM, consolidatedPassagens, consolidatedConcessionarias } = useMemo(() => {
    ...
    // CORREÇÃO: A chave do grupo agora usa strings normalizadas
    const initializeGroup = (om: string, ug: string) => {
      const omKey = `${normalizeString(om)} (${normalizeString(ug)})`;
      if (!groups[omKey]) {
    ...
    registrosPassagem.forEach(registro => {
      ...
      const consolidationKey = [
        normalizeString(registro.organizacao),
        normalizeString(registro.ug),
        normalizeString(registro.om_detentora),
        normalizeString(registro.ug_detentora),
      ...
      if (!consolidatedPassagensMap[consolidationKey]) {
        consolidatedPassagensMap[consolidationKey] = {
          ...
          organizacao: normalizeString(registro.organizacao),
          ug: normalizeString(registro.ug),
          om_detentora: normalizeString(omDetentora), // CORREÇÃO: Normalizando aqui
          ug_detentora: normalizeString(ugDetentora), // CORREÇÃO: Normalizando aqui
    ...
    registrosConcessionaria.forEach(registro => {
      ...
      if (!consolidatedConcessionariasMap[consolidationKey]) {
        consolidatedConcessionariasMap[consolidationKey] = {
          groupKey: consolidationKey,
          organizacao: normalizeString(registro.organizacao), // CORREÇÃO: Normalizando aqui
          ug: normalizeString(registro.ug), // CORREÇÃO: Normalizando aqui
          om_detentora: normalizeString(omDetentora), // CORREÇÃO: Normalizando aqui
          ug_detentora: normalizeString(ugDetentora), // CORREÇÃO: Normalizando aqui
          dias_operacao: registro.dias_operacao,
          efetivo: registro.efetivo || 0,
  ...
  }, [ptrabData, totaisND, fileSuffix, diasOperacao, generateDiariaMemoriaCalculo, diretrizesOperacionais, toast, registrosAgrupadosPorOM, consolidatedPassagensWithDetails, consolidatedConcessionariasWithDetails]);
  ...
  const exportExcel = useCallback(async () => {
    ...
    Object.entries(registrosAgrupadosPorOM).forEach(([omKey, group]) => {
      // CORREÇÃO: Normalizando a extração da chave para garantir a comparação
      const omName = normalizeString(omKey.split(' (')[0]);
      const ug = normalizeString(omKey.split(' (')[1].replace(')', ''));
      ...
      // CORREÇÃO: Normalizando os campos de comparação
      const passagensConsolidadasDesteGrupo = consolidatedPassagensWithDetails.filter(c =>
        normalizeString(c.om_detentora) === omName && normalizeString(c.ug_detentora) === ug
      );
            ...
            <table className="ptrab-table-op">
              ...
              <tbody>
                {Object.entries(registrosAgrupadosPorOM).map(([omKey, group]) => {
                  ...
                  group.passagens.forEach(r => { // Passagens (ND 33.90.33)
                    subtotalOM.nd33 += r.valor_nd_33;
                    subtotalOM.totalGND3 += r.valor_nd_33;
                  });

                  group.concessionarias.forEach(r => { // Concessionária (ND 33.90.39) // NOVO
                    subtotalOM.nd39 += r.valor_nd_39;
                    subtotalOM.totalGND3 += r.valor_nd_39;
                  });
