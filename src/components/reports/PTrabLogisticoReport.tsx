...
const PTrabLogisticoReport: React.FC<PTrabLogisticoReportProps> = ({
  ...
  const exportExcel = useCallback(async () => {
    ...
    const dataLeftMiddleAlignment = { horizontal: 'left' as const, vertical: 'middle' as const, wrapText: true }; // NOVO: Alinhamento Esquerda/Meio
    ...
    try {
      ...
      omsOrdenadas.forEach((nomeOM) => {
        ...
        allExpenseLines.forEach((linha, index) => {
            ...
            // Estilos
            ['A', 'B'].forEach(col => {
                row.getCell(col).alignment = dataLeftMiddleAlignment; // <-- ALTERADO PARA MIDDLE
