        ...
        return [
          ...
          ...linhasDespesaOrdenadas.map((linha) => {
            ...
            if (isClasseI) { // Classe I (QS/QR)
                ...
                // --- Lógica de Exibição da Coluna DESPESAS ---
                
                // 1. Base format: CLASSE X - CATEGORIA (para II, V, VI) ou CLASSE X \n CATEGORIA (para VII, VIII, IX)
                if (CLASSE_V_CATEGORIES.includes(registro.categoria) || CLASSE_VI_CATEGORIES.includes(registro.categoria) || ['Equipamento Individual', 'Proteção Balística', 'Material de Estacionamento'].includes(registro.categoria)) {
                ...
                // 2. Adicionar OM Detentora se:
                // a) É Classe VI (sempre mostrar Detentora para rastreabilidade)
                // b) É qualquer outra classe e a Detentora é diferente do Destino
                if (CLASSE_VI_CATEGORIES.includes(registro.categoria) || isDifferentOm) {
                    rowData.despesasValue += `\n${omDetentora}`;
                }
                
                // --- Lógica de Memória de Cálculo ---
