...
const fetchPTrabTotals = async (ptrabId: string) => {
  ...
  const [
    ...
    supabase
      ...
      .select('valor_total, itens_equipamentos, dias_operacao, organizacao, categoria, valor_nd_30, valor_nd_39, efetivo')
      .eq('p_trab_id', ptrabId),
    supabase
    ...
    supabase
      ...
      .select('valor_total, tipo_combustivel, total_litros, tipo_equipamento, preco_lubrificante, consumo_lubrificante_litro')
      .eq('p_trab_id', ptrabId),
    supabase
  ...
  (allClasseItemsData || []).forEach(record => {
    ...
    // CORREÇÃO: Cast para ItemClasseII[]
