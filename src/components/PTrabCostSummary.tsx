...
export const PTrabCostSummary = ({ ptrabId, onOpenCreditDialog, creditGND3, creditGND4 }: { ptrabId: string, onOpenCreditDialog: () => void, creditGND3: number, creditGND4: number }) => {
  ...
  const totalGND3 = (totals.totalLogisticoGeral || 0) + (totals.totalOperacional || 0) + (totals.totalAviacaoExercito || 0);
  const totalGND4 = totals.totalMaterialPermanente || 0;
  
  // Verificação de segurança para omBreakdown
  const omBreakdownList = totals.omBreakdown ? Object.values(totals.omBreakdown) : [];
      ...
      <Card className="border-none shadow-lg bg-gradient-to-br from-purple-50 to-white">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            {omBreakdownList.filter(om => om.totalAviacaoExercito > 0).map(om => (
              <div key={om.ug} className="space-y-2">
                ...
                <div className="space-y-1.5 border-l-2 border-purple-200 pl-3 py-1">
                  ...
                  {om.groupedHV && Object.entries(om.groupedHV).map(([tipo, data]) => (
