...
import { Package, Fuel, Utensils, Loader2, ChevronDown, HardHat, Plane, TrendingUp, Wallet, ClipboardList, Swords, Radio, Activity, HeartPulse, Truck, Briefcase, Droplet, Zap, MapPin, Building2 } from "lucide-react";
import {
  Accordion,
  AccordionItem,
  AccordionTrigger,
...
// Função de normalização profunda para chaves de OM
...
const fetchPTrabTotals = async (ptrabId: string): Promise<PTrabAggregatedTotals> => {
  try {
    ...
    const getOmTotals = (omName: string, ug: string, mode: 'solicitante' | 'destino'): OmTotals => {
        ...
        if (!targetGroup[key]) {
            targetGroup[key] = initializeOmTotals(cleanName, cleanUg);
        } else {
            const currentUgs = targetGroup[key].ug.split(', ');
            if (cleanUg && !currentUgs.includes(cleanUg)) {
                targetGroup[key].ug = [...currentUgs, cleanUg].join(', ');
            }
        }
    ...
    const processGenericClass = (data: any[], classe: string) => {
        data.forEach(record => {
            ...
            [omS, omD].forEach(omTotals => {
                ...
                const totalItens = items.reduce((sum, item) => sum + (Number(item.quantidade) || 0), 0);
                group.totalItens += totalItens;
                
                let catKey = record.categoria || (classe === 'VIII' ? 'Saúde' : 'Geral');
                if (classe === 'VIII' && record.animal_tipo) catKey = `Remonta - ${record.animal_tipo}`;
    ...
    processGenericClass(classeIIData || [], 'II');
    processGenericClass(classeVData || [], 'V');
    processGenericClass(classeVIData || [], 'VI');
    processGenericClass(classeVIIData || [], 'VII');
    processGenericClass([...(classeVIIISaudeData || []), ...(classeVIIIRemontaData || [])], 'VIII');
    processGenericClass(classeIXData || [], 'IX');
    ...
    (diariaData || []).forEach(record => {
        ...
        [omS, omD].forEach(omTotals => {
            ...
            omTotals.diarias.totalND15 += totalGeral - Number(record.valor_taxa_embarque || 0);
            omTotals.diarias.totalND30 += Number(record.valor_taxa_embarque || 0) + valorND30;
    ...
    (concessionariaData || []).forEach(record => {
        ...
        [omS, omD].forEach(omTotals => {
            ...
            if (record.categoria === 'Água/Esgoto') omTotals.concessionaria.totalAgua += valorND39;
            else if (record.categoria === 'Energia Elétrica') omTotals.concessionaria.totalEnergia += valorND39;
        });
    });
    ...
    (horasVooData || []).forEach(record => {
        ...
        [omS, omD].forEach(omTotals => {
            ...
            if (!omTotals.horasVoo.groupedHV[tipoAnv]) omTotals.horasVoo.groupedHV[tipoAnv] = { totalValor: 0, totalHV: 0 };
...
// Componente auxiliar para renderizar os detalhes de uma aba
...
const getHorasVooData = (data: OmTotals | PTrabAggregatedTotals): OmTotals['horasVoo'] => {
    ...
    const g = data as PTrabAggregatedTotals;
    return { total: g.totalHorasVoo, totalND30: g.totalHorasVooND30, totalND39: g.totalHorasVooND39, quantidadeHV: g.quantidadeHorasVoo, groupedHV: g.groupedHV };
};
      ...
      if (viewMode === 'global') {
          ...
          return (
              <div className="w-full space-y-1 text-sm px-6 pt-3">
                  ...
                  {/* Botões de Alternância de Agrupamento */}
      ...
      <CardContent className="space-y-4 p-0 pb-3">
        ...
        <Accordion type="single" collapsible className="w-full px-6 pt-0" value={isDetailsOpen ? "summary-details" : undefined} onValueChange={(v) => viewMode === 'global' && setIsDetailsOpen(v === "summary-details")}>
          <AccordionItem value="summary-details" className="border-b-0">
            <AccordionTrigger simple className="py-0 px-0 hover:no-underline flex items-center justify-between w-full text-xs text-muted-foreground border-t border-border/50" onClick={(e) => { e.preventDefault(); if (viewMode === 'global') handleSummaryClick(); }}>
