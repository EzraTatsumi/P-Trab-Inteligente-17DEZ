...
const MaterialConsumoDiretrizFormDialog: React.FC<MaterialConsumoDiretrizFormDialogProps> = ({
    ...
    useEffect(() => {
        if (diretrizToEdit) {
            setSubitemForm({ ...diretrizToEdit, itens_aquisicao: diretrizToEdit.itens_aquisicao || [] });
        } else {
            setSubitemForm({ nr_subitem: '', nome_subitem: '', descricao_subitem: '', itens_aquisicao: [], ativo: true });
        }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
                ...
                <div className="space-y-6 py-2">
                    <Card className="p-4 tour-dados-subitem">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-2 col-span-2"><Label>Nome do Subitem *</Label><Input value={subitemForm.nome_subitem} onChange={(e) => setSubitemForm({ ...subitemForm, nome_subitem: e.target.value })} placeholder="Ex: Material p/ Manutenção de Bens Imóveis" disabled={loading} /></div>
                        </div>
                    </Card>
                    <Card className="p-4 space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                <Button type="button" variant="secondary" size="sm" onClick={handleOpenPNCPSearch} disabled={loading} className="btn-importar-pncp"><Search className="h-4 w-4 mr-2" />Importar via API PNCP</Button>
                        ...
                        {(subitemForm.itens_aquisicao || []).length > 0 && (
                            <div className="tabela-itens-aquisicao">
                                <Table>
            ...
            <ItemAquisicaoPNCPDialog open={isPNCPSearchOpen} onOpenChange={setIsPNCPSearchOpen} onImport={(items) => setSubitemForm(p => ({ ...p, itens_aquisicao: [...(p.itens_aquisicao || []), ...items] }))} existingItemsInDiretriz={subitemForm.itens_aquisicao || []} onReviewItem={handleEditItem} selectedYear={selectedYear} />
        </Dialog>
    );
};
