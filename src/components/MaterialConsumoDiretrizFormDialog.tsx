...
const MaterialConsumoDiretrizFormDialog: React.FC<MaterialConsumoDiretrizFormDialogProps> = ({
    ...
    // Detecta se o modo fantasma (Tour) está ativo
    const isTourActive = typeof window !== 'undefined' && localStorage.getItem('is_ghost_mode') === 'true';

    const getInitialFormState = (editData: DiretrizMaterialConsumo | null) => {
        if (editData) return { ...editData, itens_aquisicao: editData.itens_aquisicao || [] };
    ...
    return (
        <Dialog 
            ...
            modal={!isTourActive} // Desativa o focus trap durante o Tour
        >
                            ...
                            <Table>
                                ...
                                <TableBody>{subitemForm.itens_aquisicao.map(item => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="ghost" size="icon" onClick={() => { setEditingItemId(item.id); setItemForm({ ...item, rawValor: numberToRawDigits(item.valor_unitario), descricao_reduzida: item.descricao_reduzida || '' }); itemFormRef.current?.scrollIntoView({ behavior: 'smooth' }); }}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => setSubitemForm(p => ({ ...p, itens_aquisicao: p.itens_aquisicao.filter(i => i.id !== item.id) }))} className="text-destructive"><Trash2 className="h-4 w-4" /></Button></div></TableCell>
                                    </TableRow>
                                ))}</TableBody>
                            </Table>
                        )}
