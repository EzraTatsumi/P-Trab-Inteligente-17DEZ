...
export async function exportServicosTerceirosToExcel(diretrizes: DiretrizServicosTerceiros[], year: number) {
    ...
    const worksheet = workbook.addWorksheet(`Servicos_${year}`);
    ...
    worksheet.columns = [
        ...
        { header: 'CODIGO_CATSER', key: 'codigo_catmat', width: 20 },
    ...
    saveAs(new Blob([buffer]), `Diretrizes_ServicosTerceiros_${year}.xlsx`);
}
    ...
    worksheet?.eachRow((row, rowNumber) => {
        ...
        if (nr_subitem && nome_subitem && valor_unitario > 0) {
            stagedData.push({
                ...
                valor_unitario,
                numero_pregao: String(row.getCell(9).value || ''),
                uasg: String(row.getCell(10).value || ''),
                isValid: true,
                errors: [],
