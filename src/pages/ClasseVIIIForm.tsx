...
interface ItemClasseVIII {
  item: string;
  quantidade: number;
  valor_unitario: number; // Valor unitário (não por dia)
  categoria: string;
...
interface FormDataClasseVIII {
  ...
  dias_operacao: number; // Global (apenas para detalhamento)
...
interface ClasseVIIIRegistro {
  ...
  categoria: string;
  itens_equipamentos: ItemClasseVIII[]; // Tipo corrigido
  valor_total: number;
  detalhamento: string;
...
// NOVO: Função para gerar a memória de cálculo da Classe VIII
const generateDetalhamento = (itens: ItemClasseVIII[], diasOperacao: number, organizacao: string, ug: string, faseAtividade: string, omDestino: string, ugDestino: string, valorND30: number, valorND39: number): string => {
    const faseFormatada = formatFasesParaTexto(faseAtividade);
    const totalItens = itens.reduce((sum, item) => sum + item.quantidade, 0);
    const valorTotal = valorND30 + valorND39;
    const categoriaPrincipal = itens[0]?.categoria || 'Classe VIII';

    const gruposPorCategoria = itens.reduce((acc, item) => {
        const categoria = item.categoria;
        const valorItem = item.quantidade * item.valor_unitario; // Cálculo sem dias
        ...
        acc[categoria].detalhes.push(
            `- ${item.quantidade} ${item.item} x ${formatCurrency(item.valor_unitario)} = ${formatCurrency(valorItem)}.`
        );
    ...
    // Adaptação da descrição para o exemplo fornecido
    let descricaoBase = `33.90.30 / 33.90.39 - Aquisição de Material de ${categoriaPrincipal} (Diversos) para utilização por ${totalItens} itens, durante ${diasOperacao} dias de ${faseFormatada}, para ${organizacao}.`;
    
    if (categoriaPrincipal === 'Saúde') {
        // Usando o texto base do exemplo fornecido, mas parametrizado
        descricaoBase = `33.90.30 - Aquisição de KPSI/KPSC e KPT para utilização por ${totalItens} militares do ${organizacao}, durante a ${faseFormatada}.`;
    }
...
Cálculo:
Fórmula Base: Nr Itens x Valor Unitário.
...
const ClasseVIIIForm = () => {
  ...
  useEffect(() => {
    if (diretrizes.length > 0 && form.organizacao) {
        // Mapeia diretrizes para ItemClasseVIII (usando valor_mnt_dia como valor_unitario)
        const availableItems = diretrizes
            .filter(d => d.categoria === selectedTab)
            .map(d => ({
                item: d.item,
                quantidade: 0,
                valor_unitario: Number(d.valor_mnt_dia), // Usar valor_mnt_dia como valor_unitario
  ...
  const fetchRegistros = async () => {
    ...
    const { data, error } = await supabase
      .from("classe_viii_registros") // NOVO: Tabela Classe VIII
    ...
    const uniqueRecordsMap = new Map<string, ClasseVIIIRegistro>();
    (data || []).forEach(r => {
        const key = `${r.organizacao}-${r.ug}-${r.categoria}`;
        const record = {
            ...r,
            itens_equipamentos: (r.itens_equipamentos || []) as ItemClasseVIII[],
  ...
  // CÁLCULO: Total da categoria é Qtd * Valor Unitário (sem dias)
  const currentCategoryTotalValue = currentCategoryItems.reduce((sum, item) => sum + (item.quantidade * item.valor_unitario), 0);
  ...
  const handleUpdateCategoryItems = () => {
    ...
    const categoryTotalValue = currentCategoryItems.reduce((sum, item) => sum + (item.quantidade * item.valor_unitario), 0);
  ...
  const handleSalvarRegistros = async () => {
    ...
    const registrosParaSalvar: TablesInsert<'classe_viii_registros'>[] = [];
    ...
    for (const categoria of categoriesToSave) {
        ...
        const valorTotalCategoria = itens.reduce((sum, item) => sum + (item.quantidade * item.valor_unitario), 0);
        ...
        const registro: TablesInsert<'classe_viii_registros'> = {
            p_trab_id: ptrabId,
    ...
    try {
      // 3. Deletar APENAS registros de Classe VIII existentes para o PTrab
      const { error: deleteError } = await supabase
        .from("classe_viii_registros")
        .delete()
        .eq("p_trab_id", ptrabId);
      ...
      const { error: insertError } = await supabase.from("classe_viii_registros").insert(registrosParaSalvar);
  ...
  const handleEditarRegistro = async (registro: ClasseVIIIRegistro) => {
    ...
    // 1. Buscar TODOS os registros de CLASSE VIII para este PTrab
    ...
    (allRecords || []).forEach(r => {
        ...
        const items = (r.itens_equipamentos || []) as ItemClasseVIII[];
        
        consolidatedItems = consolidatedItems.concat(items);
        
        if (newAllocations[category]) {
            const valorTotal = items.reduce((sum, item) => sum + (item.quantidade * item.valor_unitario), 0);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Solicitação de recursos para aquisição de Kits de Saúde (KPSI/KPSC/KPT) e material de Remonta/Veterinária.
            </CardDescription>
          </CardHeader>
            ...
            <div className="space-y-3 border-b pb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Usado apenas no detalhamento da memória.</p>
                </div>
              </div>
              ...
              <div className="space-y-4 border-b pb-4">
                ...
                <div className="space-y-4">
                  {Object.entries(itensAgrupadosPorCategoria).map(([categoria, itens]) => {
                    const totalCategoria = itens.reduce((sum, item) => sum + (item.quantidade * item.valor_unitario), 0);
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={() => {
                                                                if (confirm(`Deseja realmente deletar o registro de Classe VIII para ${omName} (${registro.categoria})?`)) {
                                                                    supabase.from("classe_viii_registros")
                                                                        .delete()
                                                                        .eq("id", registro.id)
                                                                        .then(() => {
              ...
              <div className="space-y-4 mt-8">
                ...
                {registros.map(registro => {
                  ...
                  const memoriaAutomatica = generateDetalhamento(
                      registro.itens_equipamentos as ItemClasseVIII[], 
