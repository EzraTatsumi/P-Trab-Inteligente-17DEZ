...
import { ArrowLeft, Pencil, Trash2, XCircle, Check, ChevronsUpDown, Sparkles, AlertCircle, Car } from "lucide-react";
...
interface ItemClasseIX {
  item: string;
  quantidade: number;
  valor_mnt_dia: number; // Valor de Manutenção/Dia
  valor_acionamento_mensal: number; // Valor de Acionamento Mensal
  categoria: string;
...
// Helper function to calculate total cost for a single item (Daily + Monthly Activation)
const calculateItemTotal = (item: ItemClasseIX, diasOperacao: number): number => {
    if (item.quantidade <= 0 || diasOperacao <= 0) return 0;
    
    // Número de meses de acionamento (arredondado para cima)
    const nrMeses = Math.ceil(diasOperacao / 30);
    
    const dailyCost = item.quantidade * item.valor_mnt_dia * diasOperacao;
    const monthlyActivationCost = item.quantidade * item.valor_acionamento_mensal * nrMeses;
    
    return dailyCost + monthlyActivationCost;
};
...
const generateCategoryMemoriaCalculo = (categoria: Categoria, itens: ItemClasseIX[], diasOperacao: number, organizacao: string, ug: string, faseAtividade: string | null | undefined): string => {
    ...
    const totalValor = itens.reduce((sum, item) => sum + calculateItemTotal(item, diasOperacao), 0);
    ...
    itens.forEach(item => {
        const dailyCost = item.quantidade * item.valor_mnt_dia * diasOperacao;
        const monthlyActivationCost = item.quantidade * item.valor_acionamento_mensal * nrMeses;
        const itemTotal = dailyCost + monthlyActivationCost;
        
        totalDailyCost += dailyCost;
        totalMonthlyCost += monthlyActivationCost;

        detalhamentoItens += `- ${item.quantidade} ${item.item}:\n`;
        detalhamentoItens += `  - Custo Diário: ${item.quantidade} un. x ${formatCurrency(item.valor_mnt_dia)}/dia x ${diasOperacao} dias = ${formatCurrency(dailyCost)}.\n`;
        detalhamentoItens += `  - Custo Mensal: ${item.quantidade} un. x ${formatCurrency(item.valor_acionamento_mensal)}/mês x ${nrMeses} meses = ${formatCurrency(monthlyActivationCost)}.\n`;
        detalhamentoItens += `  - Total Item: ${formatCurrency(itemTotal)}.\n`;
    });

    return `33.90.30 / 33.90.39 - Aquisição de Material de Classe IX (Motomecanização - ${categoria})
OM Detentora: ${organizacao} (UG: ${ug})
Período: ${diasOperacao} dias de ${faseFormatada} (${nrMeses} meses de acionamento)
...
Resumo de Custos:
- Total Custo Diário: ${formatCurrency(totalDailyCost)}.
- Total Custo Mensal (Acionamento): ${formatCurrency(totalMonthlyCost)}.

Valor Total Solicitado: ${formatCurrency(totalValor)}.`;
};
...
const ClasseIXForm = () => {
  ...
  const loadDiretrizes = async () => {
    try {
      ...
      if (classeIXData && classeIXData.length > 0) {
        setDiretrizes((classeIXData || []).map(d => ({
            ...d,
            valor_mnt_dia: Number(d.valor_mnt_dia),
            valor_acionamento_mensal: Number(d.valor_acionamento_mensal),
        })) as DiretrizClasseIX[]);
  ...
  // CÁLCULO BASE
  const currentCategoryTotalValue = currentCategoryItems.reduce((sum, item) => sum + calculateItemTotal(item, form.dias_operacao), 0);
  ...
  const valorTotalForm = form.itens.reduce((sum, item) => sum + calculateItemTotal(item, form.dias_operacao), 0);
  ...
  const handleSalvarRegistros = async () => {
    ...
    for (const categoria of categoriesToSave) {
        ...
        const valorTotalCategoria = itens.reduce((sum, item) => sum + calculateItemTotal(item, form.dias_operacao), 0);
        ...
        const registro: TablesInsert<'classe_ix_registros'> = {
            ...
            categoria: categoria,
            itens_motomecanizacao: itens as any, // Usando o novo campo JSONB
    ...
    try {
      // Deletar APENAS registros de Classe IX existentes para o PTrab
  ...
  const handleEditarRegistro = async (registro: ClasseIXRegistro) => {
    ...
    (allRecords || []).forEach(r => {
        ...
        if (newAllocations[category]) {
            const totalValor = items.reduce((sum, item) => sum + calculateItemTotal(item, r.dias_operacao), 0);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Solicitação de recursos para manutenção de viaturas (custo diário + acionamento mensal).
            </CardDescription>
          </CardHeader>
            ...
            <div className="space-y-3 border-b pb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label>OM Detentora da Viatura *</Label>
                  <OmSelector
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Calcula {Math.ceil(form.dias_operacao / 30)} meses de acionamento.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4 border-b pb-4" ref={formRef}>
                ...
                <Tabs value={selectedTab} onValueChange={(value) => setSelectedTab(value as Categoria)}>
                  <TabsList className="grid w-full grid-cols-4">
                    {CATEGORIAS.map(cat => (
                      <TabsTrigger key={cat} value={cat}>{cat.replace('Vtr ', '')}</TabsTrigger>
                    ))}
                  </TabsList>
                            ...
                            <Table className="w-full">
                                <TableHeader className="sticky top-0 bg-muted/80 backdrop-blur-sm z-10">
                                    <TableRow>
                                        ...
                                        <TableHead className="w-[20%] text-right">Total Custo</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {currentCategoryItems.length === 0 ? (
                                        ...
                                        currentCategoryItems.map((item, index) => {
                                            const itemTotal = calculateItemTotal(item, form.dias_operacao);
              ...
              <div className="space-y-4 border-b pb-4">
                ...
                <div className="space-y-4">
                  {Object.entries(itensAgrupadosPorCategoria).map(([categoria, itens]) => {
                    const totalCategoria = itens.reduce((sum, item) => sum + calculateItemTotal(item, form.dias_operacao), 0);
                    const totalQuantidade = itens.reduce((sum, item) => sum + item.quantidade, 0);
                    ...
                    return (
                      <Card key={categoria} className="p-4 bg-secondary/10 border-secondary">
                        <div className="flex items-center justify-between mb-3 border-b pb-2">
                          <h4 className="font-bold text-base text-primary">{categoria} ({totalQuantidade} viaturas)</h4>
                        ...
                        <div className="space-y-2">
                          {itens.map((item, index) => {
                            const itemTotal = calculateItemTotal(item, form.dias_operacao);
                            const dailyCost = item.quantidade * item.valor_mnt_dia * form.dias_operacao;
                            const monthlyActivationCost = item.quantidade * item.valor_acionamento_mensal * nrMeses;
                            
                            return (
                              <div key={index} className="flex flex-col text-sm text-muted-foreground border-b border-dashed pb-1 last:border-b-0 last:pb-0">
                                <span className="font-medium text-foreground">{item.item}</span>
                                <div className="flex justify-between text-xs mt-1">
                                    <span>Custo Diário: {formatCurrency(dailyCost)}</span>
                                    <span>Acionamento Mensal: {formatCurrency(monthlyActivationCost)}</span>
                                    <span className="font-semibold text-right text-primary">Total: {formatCurrency(itemTotal)}</span>
                                </div>
                              </div>
                            );
                          })}
