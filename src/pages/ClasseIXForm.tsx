...
// Helper para calcular o valor total de um item da Classe IX
const calculateItemTotal = (item: ItemClasseIX, diasOperacao: number): number => {
    if (item.quantidade <= 0 || diasOperacao <= 0) return 0;
    
    // Cálculo: (Valor Mnt/Dia * Dias Operação) + (Valor Acionamento Mensal * Nr Meses)
    const valorManutencao = item.valor_mnt_dia * diasOperacao * item.quantidade;
    
    // Calcula o número de meses completos ou parciais
    const nrMeses = Math.ceil(diasOperacao / 30);
    const valorAcionamento = item.valor_acionamento_mensal * nrMeses * item.quantidade;
    
    return valorManutencao + valorAcionamento;
};

// NOVO: Gera a memória de cálculo detalhada para a categoria
const generateDetalhamento = (itens: ItemClasseIX[], diasOperacao: number, organizacao: string, ug: string, faseAtividade: string, omDestino: string, ugDestino: string, valorND30: number, valorND39: number): string => {
    ...
    const gruposPorCategoria = itens.reduce((acc, item) => {
        ...
        const valorItem = calculateItemTotal(item, diasOperacao);
        ...
        acc[categoria].detalhes.push(
            `- ${item.quantidade} ${item.item}:
              - Mnt/Dia: ${item.quantidade} un. x ${formatCurrency(item.valor_mnt_dia)}/dia x ${diasOperacao} dias = ${formatCurrency(valorManutencao)}.
              - Acionamento: ${item.quantidade} un. x ${formatCurrency(item.valor_acionamento_mensal)}/mês x ${nrMeses} meses = ${formatCurrency(valorAcionamento)}.
              - Total Item: ${formatCurrency(valorItem)}.`
        );
    ...
    return `33.90.30 / 33.90.39 - Aquisição de Material de Motomecanização (Classe IX) para ${totalItens} itens, durante ${diasOperacao} dias de ${faseFormatada}, para ${organizacao}.
...
Cálculo:
Fórmula Base: (Nr Itens x Valor Mnt/Dia x Nr Dias) + (Nr Itens x Valor Acionamento Mensal x Nr Meses).
...
const ClasseIXForm = () => {
  ...
  }, [selectedTab, diretrizes, form.itens]);
  ...
  // CÁLCULO: Valor total da categoria (Manutenção + Acionamento)
  const currentCategoryTotalValue = useMemo(() => {
    return currentCategoryItems.reduce((sum, item) => sum + calculateItemTotal(item, form.dias_operacao), 0);
  ...
  // CÁLCULO GLOBAL
  const valorTotalForm = Object.values(categoryAllocations).reduce((sum, alloc) => alloc.total_valor, 0);
  ...
  const getCategoryIcon = (category: Categoria) => {
    switch (category) {
  ...
  const getCategoryBadgeStyle = (category: Categoria) => {
    switch (category) {
        case 'Vtr Administrativa': return 'bg-blue-600 hover:bg-blue-700 text-white';
        case 'Vtr Operacional': return 'bg-green-600 hover:bg-green-700 text-white';
        case 'Motocicleta': return 'bg-yellow-600 hover:bg-yellow-700 text-white';
        case 'Vtr Blindada': return 'bg-red-600 hover:bg-red-700 text-white';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Solicitação de recursos para manutenção de viaturas e equipamentos de motomecanização.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {/* 2. Configurar Item por Categoria (Aba) */}
            {form.organizacao && form.dias_operacao > 0 && (
              <div className="space-y-4 border-b pb-4">
                <h3 className="text-lg font-semibold">2. Configurar Item por Categoria</h3>
                            ...
                            <Table className="w-full">
                                <TableHeader className="sticky top-0 bg-muted/80 backdrop-blur-sm z-10">
                                    <TableRow>
                                        ...
                                        <TableHead className="w-[15%] text-right">Mnt/Dia</TableHead>
                            ...
                            return (
                              <div key={index} className="flex justify-between text-sm text-muted-foreground border-b border-dashed pb-1 last:border-b-0 last:pb-0">
                                ...
                                <span className="text-right">
                                  {item.quantidade} un. x ({formatCurrency(item.valor_mnt_dia)}/dia x {form.dias_operacao} dias + {formatCurrency(item.valor_acionamento_mensal)}/mês x {nrMeses} meses) = {formatCurrency(itemTotal)}
                                </span>
                              </div>
                            );
                          })}
                        ...
                        <Card key={omKey} className="p-4 bg-primary/5 border-primary/20">
                            ...
                            <div className="space-y-3">
                                {omRegistros.map((registro) => {
                                    ...
                                    const badgeClass = getCategoryBadgeStyle(registro.categoria as Categoria);
              ...
              <div className="space-y-4 mt-8">
                ...
                {registros.map(registro => {
                  ...
                  const memoriaAutomatica = generateDetalhamento(
                      registro.itens_motomecanizacao, 
