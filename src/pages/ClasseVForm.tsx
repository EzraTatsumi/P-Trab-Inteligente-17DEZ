...
const generateClasseIIMemoriaCalculo = (registro: ClasseIIRegistro): string => {
  ...
  const memoria = `33.90.30/33.90.39 - Aquisição de Material de Armamento (Classe V) para ${organizacao} (UG: ${ug}), durante ${dias_operacao} dias de ${faseFormatada}.
...
export default function ClasseVForm() {
  ...
  const loadDiretrizes = async () => {
    try {
      ...
      if (!anoReferencia) {
        setDiretrizAno(null);
        setDiretrizesDisponiveis(defaultClasseVConfig as DiretrizClasseII[]);
        return;
      }
      
      setDiretrizAno(anoReferencia);

      // 3. Buscar diretrizes de Classe V
      const { data, error } = await supabase
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <Card ref={formRef}>
          <CardHeader>
            <CardTitle>Classe V - Armamento</CardTitle>
            <CardDescription>
              Configure a necessidade de material de Armamento por OM.
            </CardDescription>
          </CardHeader>
