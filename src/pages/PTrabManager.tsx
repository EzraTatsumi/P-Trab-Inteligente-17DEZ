...
const PTrabManager = () => {
  ...
  // --- LÓGICA UNIFICADA DE VITÓRIA E SUPRESSÃO DE MODAIS ---
  useEffect(() => {
    if (!user?.id) return;
    ...
    // Checagem Ativa (ao carregar a página)
    if (shouldShowVictory(user.id)) {
      ativarVitoria();
    }

    // Checagem Reativa (evento em tempo real)
  ...
  useEffect(() => {
    // Impede o WelcomeModal de aparecer se o Hub ou a Vitória estiverem ativos
  ...
  const { data: pTrabs = [], isLoading: loading, refetch: loadPTrabs } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("p_trab").select("*, comentario, origem, rotulo_versao, user_id, shared_with, share_token").or(`user_id.eq.${user.id},shared_with.cs.{${user.id}}`).order("created_at", { ascending: false });
  ...
  const handleEdit = (ptrab: PTrab) => {
    ...
    setFormData({ ...ptrab, comentário: ptrab.comentario || "", ...ptrab });
    setDialogOpen(true);
  };
