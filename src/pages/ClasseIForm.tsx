...
import { DiretrizClasseI } from "@/types/diretrizesClasseI";
...
import { getOmPreposition } from "@/lib/classeIUtils";
import { generateClasseIMemoriaCalculo } from "@/lib/classeIUtils";

type Categoria = 'Viaturas' | 'Aeronaves' | 'Embarcações' | 'Motomecanização';
...
interface ClasseIRegistro {
  ...
  categoria: string;
  itens_equipamentos: ItemClasseI[]; // Tipo corrigido
  valor_total: number;
  detalhamento: string;
...
const initialCategoryAllocations: Record<Categoria, CategoryAllocation> = {
    'Viaturas': { total_valor: 0, nd_39_input: "", nd_30_value: 0, nd_39_value: 0, om_destino_recurso: "", ug_destino_recurso: "", selectedOmDestinoId: undefined },
    'Aeronaves': { total_valor: 0, nd_39_input: "", nd_30_value: 0, nd_39_value: 0, om_destino_recurso: "", ug_destino_recurso: "", selectedOmDestinoId: undefined },
    'Embarcações': { total_valor: 0, nd_39_input: "", nd_30_value: 0, nd_39_value: 0, om_destino_recurso: "", ug_destino_recurso: "", selectedOmDestinoId: undefined },
    'Motomecanização': { total_valor: 0, nd_39_input: "", nd_30_value: 0, nd_39_value: 0, om_destino_recurso: "", ug_destino_recurso: "", selectedOmDestinoId: undefined },
};
...
const groupRecordItemsByCategory = (items: ItemClasseI[]) => {
    return items.reduce((acc, item) => {
        ...
        return acc;
    }, {} as Record<Categoria, ItemClasseI[]>);
};
...
// A lógica foi movida para generateClasseIMemoriaCalculo em src/lib/classeIUtils.ts
...
const ClasseIForm = () => {
  ...
  const [diretrizes, setDiretrizes] = useState<DiretrizClasseI[]>([]);
  ...
  const [form, setForm] = useState<FormDataClasseI>({
  ...
  const [currentCategoryItems, setCurrentCategoryItems] = useState<ItemClasseI[]>([]);
  ...
  useEffect(() => {
    ...
    if (diretrizes.length > 0 && form.organizacao) {
        ...
        const existingItemsMap = new Map<string, ItemClasseI>();
  ...
  const loadDiretrizes = async () => {
    try {
      ...
      if (!anoReferencia) {
        ...
        setDiretrizes(defaultClasseIConfig as DiretrizClasseI[]);
        return;
      }

      // 3. Buscar diretrizes de Classe I usando o ano de referência encontrado
      const { data: classeIData, error } = await supabase
        .from("diretrizes_classe_i")
        .select("*")
        .eq("user_id", user.id)
      ...
      if (classeIData && classeIData.length > 0) {
        setDiretrizes((classeIData || []) as DiretrizClasseI[]);
      } else {
        setDiretrizes(defaultClasseIConfig as DiretrizClasseI[]);
        toast.warning(`Itens de Classe I não configurados para o ano ${anoReferencia}. Usando valores padrão.`);
      }
  ...
  const fetchRegistros = async () => {
    ...
    const uniqueRecordsMap = new Map<string, ClasseIRegistro>();
    (data || []).forEach(r => {
        const key = `${r.organizacao}-${r.ug}-${r.categoria}`; // Chave única por OM de destino E Categoria
        const record = {
            ...r,
            itens_equipamentos: (r.itens_equipamentos || []) as ItemClasseI[],
  ...
  const handleUpdateCategoryItems = () => {
    if (!form.organizacao || form.dias_operacao <= 0) {
        toast.error("Preencha a OM e Dias de Operação antes de salvar itens.");
        return;
    }
  ...
  const handleSalvarRegistros = async () => {
    ...
    const registrosParaSalvar: TablesInsert<'classe_i_registros'>[] = [];
    ...
    for (const categoria of categoriesToSave) {
        ...
        const detalhamento = generateClasseIMemoriaCalculo({
        ...
        const registro: TablesInsert<'classe_i_registros'> = {
            p_trab_id: ptrabId,
            organizacao: allocation.om_destino_recurso, // OM de destino do recurso (ND 30/39)
    ...
    try {
      // 3. Deletar APENAS registros de Classe I existentes para o PTrab
      ...
      const { error: insertError } = await supabase.from("classe_i_registros").insert(registrosParaSalvar);
      if (insertError) throw insertError;
      
      toast.success(editingId ? "Registros de Classe I atualizados com sucesso!" : "Registros de Classe I salvos com sucesso!");
      await updatePTrabStatusIfAberto(ptrabId);
      resetFormFields();
      fetchRegistros();
    } catch (error) {
      console.error("Erro ao salvar registros de Classe I:", error);
      toast.error("Erro ao salvar registros de Classe I");
    } finally {
      setLoading(false);
    }
  };

  const handleEditarRegistro = async (registro: ClasseIRegistro) => {
    setLoading(true);
    resetFormFields();
    
    // 1. Buscar TODOS os registros de CLASSE I para este PTrab
    ...
    let consolidatedItems: ItemClasseI[] = [];
    ...
    for (const r of (allRecords || [])) {
        ...
        const items = (r.itens_equipamentos || []) as ItemClasseI[];
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              Classe I - Material de Consumo
            </CardTitle>
            <CardDescription>
              Solicitação de recursos para manutenção de material de consumo (Viaturas, Aeronaves, Embarcações, Motomecanização).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3 border-b pb-4">
              ...
              {/* PRIMEIRA LINHA: OM Detentora, UG Detentora, Dias de Atividade (3 Colunas) */}
              ...
              {/* SEGUNDA LINHA: Fase da Atividade (3 Colunas: 3) */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2 md:col-span-3"> {/* Ocupa 3 colunas em telas médias */}
                        ...
                        <div className="pt-2 border-t mt-2">
                            <div className="flex justify-between text-xs">
                                ...
                                <span className="font-medium text-foreground">
                                    {allocation.om_destino_recurso} ({formatNumber(allocation.ug_destino_recurso)}) // FORMATANDO AQUI
                                </span>
                            </div>
                            <div className="flex justify-between text-xs">
            ...
            {registros.length > 0 && (
              <div className="space-y-4 mt-6">
                ...
                {Object.entries(registrosAgrupadosPorOM).map(([omKey, omRegistros]) => {
                    ...
                    const ug = omKey.split(' (')[1].replace(')', ''); // UG já formatada pelo useMemo
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="flex gap-1">
                                                        ...
                                                        <Button
                                                            ...
                                                            size="icon"
                                                            onClick={() => {
                                                                if (confirm(`Deseja realmente deletar o registro de Classe I para ${omName} (${registro.categoria})?`)) {
                                                                    supabase.from("classe_i_registros")
                                                                        .delete()
                                                                        .eq("id", registro.id)
                                                                        .then(() => {
                      ...
                      <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                              <h4 className="text-base font-semibold text-foreground">
                                OM Destino: {om} ({formatNumber(ug)}) // FORMATANDO AQUI
                              </h4>
