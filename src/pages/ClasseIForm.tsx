...
export default function ClasseIForm() {
  ...
  const [selectedOmId, setSelectedOmId] = useState<string | undefined>(
    undefined
  );
  ...
  const [initialOmName, setInitialOmName] = useState<string | undefined>(
    undefined
  );
  ...
  const [selectedTab, setSelectedTab] =
    useState<CategoriaClasseI>("RACAO_QUENTE");
  const [editingRegistroId, setEditingRegistroId] = useState<string | null>(
    null
  );
  ...
  const loadRegistros = async (ptrabId: string) => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "*, memoria_calculo_qs_customizada, memoria_calculo_qr_customizada, memoria_calculo_op_customizada, fase_atividade, categoria, quantidade_r2, quantidade_r3"
        )
      ...
      const registrosCarregados: ClasseIRegistro[] = (data || []).map((r) => {
        ...
        const isRacaoQuenteValid =
          categoria === "RACAO_QUENTE" && r.efetivo && r.nr_ref_int;
  ...
  const calculosRacaoQuente = useMemo(() => {
    ...
    return calculateClasseICalculations(
      efetivo,
      diasOperacao,
      nrRefInt,
      valorQS,
  ...
  const isConsolidationOutdated = useMemo(() => {
    ...
    const checkOutdated = (category: CategoriaClasseI): boolean => {
      ...
      if (
        consolidatedRecord.organizacao !== organizacao ||
      ...
      if (category === "RACAO_QUENTE") {
        ...
        if (calculosRacaoQuente) {
          ...
          if (
            Math.abs(
              calculosRacaoQuente.totalQS - (consolidatedRecord.total_qs || 0)
            ) > tolerance ||
            Math.abs(
              calculosRacaoQuente.totalQR - (consolidatedRecord.total_qr || 0)
            ) > tolerance
          ) {
            return true;
          }
        ...
        if (
          consolidatedRecord.quantidade_r2 !== quantidadeR2 ||
    ...
    currentOMConsolidatedData,
    organizacao,
    ug,
    diasOperacao,
    efetivo,
    getCurrentFaseFinalString,
    omQS,
    ugQS,
    nrRefInt,
    quantidadeR2,
  ...
  const handleSaveCategoryConfig = () => {
    ...
    if (selectedTab === "RACAO_QUENTE") {
      ...
      if (valorQS <= 0 || valorQR <= 0) {
        toast.error(
          "Os valores QS e QR da Diretriz de Custeio devem ser maiores que zero."
        );
        return;
      }

      const calculos = calculateClasseICalculations(
        efetivo,
        diasOperacao,
        nrRefInt,
        valorQS,
      ...
      if (totalGeral === 0) {
        toast.error(
          "O cálculo da Ração Quente resultou em zero. Verifique Efetivo, Dias e Valores."
        );
        return;
      }
  ...
  const handleFinalSave = async () => {
    ...
    if (isConsolidationOutdated) {
      toast.error(
        "A configuração da OM foi alterada. Clique em 'Salvar Item da Categoria' na(s) aba(s) ativa(s) para atualizar os cálculos antes de salvar os registros."
      );
      return;
    }
    ...
    if (currentOMConsolidatedData.RACAO_QUENTE) {
      ...
      if (r.id) {
        ...
        memoriaOpCustomizada =
          existingMemoria?.memoria_calculo_op_customizada || null;
      }
    ...
    try {
      ...
      toast.success(
        `Registro de Classe I para ${organizacao} salvo com sucesso!`
      );
  ...
  const handleRemover = async (id: string) => {
    ...
    const categoriaLabel =
      registro.categoria === "RACAO_QUENTE"
        ? "Ração Quente (QS/QR)"
        : "Ração Operacional (R2/R3)";
    if (
      !confirm(
        `Tem certeza que deseja remover o registro de ${categoriaLabel} para a OM ${registro.organizacao}?`
      )
    )
      return;
    try {
  ...
  const handleEditRegistro = async (registro: ClasseIRegistro) => {
    ...
    const fasesSalvas = (registro.faseAtividade || "Execução")
      .split(";")
      .map((f) => f.trim())
      .filter((f) => f);
    setFasesAtividade(fasesSalvas.filter((f) => FASES_PADRAO.includes(f)));
    setCustomFaseAtividade(
      fasesSalvas.find((f) => !FASES_PADRAO.includes(f)) || ""
    );
  ...
  const handleRestaurarMemoriaAutomatica = async (registroId: string) => {
    if (
      !confirm(
        "Deseja restaurar a memória de cálculo automática? O texto customizado será perdido."
      )
    )
      return;
    try {
  ...
  const totalGeralQS = registros
    .filter((r) => r.categoria === "RACAO_QUENTE")
    .reduce((sum, r) => sum + r.calculos.totalQS, 0);
  const totalGeralQR = registros
    .filter((r) => r.categoria === "RACAO_QUENTE")
    .reduce((sum, r) => sum + r.calculos.totalQR, 0);
  const totalGeral = totalGeralQS + totalGeralQR;
  const totalRacoesOperacionaisGeral = registros
    .filter((r) => r.categoria === "RACAO_OPERACIONAL")
    .reduce(
      (sum, r) => sum + (r.quantidadeR2 || 0) + (r.quantidadeR3 || 0),
      0
    );
  const displayFases = [...fasesAtividade, customFaseAtividade.trim()]
    .filter((f) => f)
    .join(", ");
  const isConfigReady =
    currentOMConsolidatedData &&
    (currentOMConsolidatedData.RACAO_QUENTE ||
      currentOMConsolidatedData.RACAO_OPERACIONAL);
  const totalMonetarioConsolidado =
    currentOMConsolidatedData?.RACAO_QUENTE?.total_geral || 0;
  const totalUnidadesConsolidado =
    currentOMConsolidatedData?.RACAO_OPERACIONAL?.total_unidades || 0;
  const RACAO_QUENTE_DATA = currentOMConsolidatedData?.RACAO_QUENTE;
  const diasEtapaSolicitada = RACAO_QUENTE_DATA
    ? calculateClasseICalculations(
        RACAO_QUENTE_DATA.efetivo,
        RACAO_QUENTE_DATA.dias_operacao,
        RACAO_QUENTE_DATA.nr_ref_int || 1,
        RACAO_QUENTE_DATA.valor_qs || valorQS,
        RACAO_QUENTE_DATA.valor_qr || valorQR
      ).diasEtapaSolicitada
        ...
        <Card className="p-6 backdrop-blur-sm bg-card/95 border-primary/10 max-w-6xl mx-auto">
          <div className="mb-4">
            ...
            <p className="text-muted-foreground">
              Configure a sua necessidade de alimentação (Ração Quente e Ração
              Operacional) por OM.
            </p>
          </div>
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <OmSelector
                      ...
                      initialOmName={initialOmName}
                      initialOmUg={initialOmUg}
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                      value={
                        diasOperacao === 0 ? "" : diasOperacao.toString()
                      }
                        ...
                        <Command>
                          ...
                          <div className="p-2 border-t">
                            <Label className="text-xs text-muted-foreground mb-1 block">
                              Outra Atividade (Opcional)
                            </Label>
                            <Input
                              value={customFaseAtividade}
                              onChange={(e) =>
                                setCustomFaseAtividade(e.target.value)
                              }
            ...
            {organizacao && diasOperacao > 0 && efetivo > 0 && (
              <div className="space-y-4 border-b pb-4">
                ...
                <Tabs
                  value={selectedTab}
                  onValueChange={(value) =>
                    setSelectedTab(value as CategoriaClasseI)
                  }
                >
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="RACAO_QUENTE">
                      Ração Quente (R1)
                    </TabsTrigger>
                    <TabsTrigger value="RACAO_OPERACIONAL">
                      Ração Operacional (R2/R3)
                    </TabsTrigger>
                  </TabsList>
                  <TabsContent value="RACAO_QUENTE" className="mt-4">
                    <div className="space-y-2 p-4 bg-muted/50 rounded-lg">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 pt-4">
                        <div className="space-y-2">
                          <Label>
                            Valor QS (Diretriz {diretrizAno || "Atual"})
                          </Label>
                          <Input
                        ...
                        <div className="space-y-2">
                          <Label>
                            Valor QR (Diretriz {diretrizAno || "Atual"})
                          </Label>
                          <Input
                      ...
                      {calculosRacaoQuente && (
                        <div className="space-y-1 mt-6 p-4 bg-background rounded-lg border">
                          <h5 className="font-semibold text-sm">
                            Previsão de Custo (Ração Quente)
                          </h5>
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-semibold text-blue-600">
                              {formatCurrency(calculosRacaoQuente.totalQS)}
                            </span>
                          </div>
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-muted-foreground">
                              Total QR (Quantitativo de Reforço)
                            </span>
                            <span className="font-semibold text-green-600">
                              {formatCurrency(calculosRacaoQuente.totalQR)}
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between items-center text-lg pt-1">
                            ...
                            <span className="font-bold text-primary">
                              {formatCurrency(
                                calculosRacaoQuente.totalQS +
                                  calculosRacaoQuente.totalQR
                              )}
                            </span>
                          </div>
                        </div>
                  ...
                  <TabsContent value="RACAO_OPERACIONAL" className="mt-4">
                    <div className="space-y-2 p-4 bg-muted/50 rounded-lg">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Informe a quantidade total de rações operacionais
                        necessárias para o efetivo e dias de atividade.
                      </p>
                              ...
                              <TableCell className="py-1">
                                <Input
                                  ...
                                  value={
                                    quantidadeR2 === 0
                                      ? ""
                                      : quantidadeR2.toString()
                                  }
                                  onChange={(e) =>
                                    setQuantidadeR2(Number(e.target.value))
                                  }
                                  placeholder="0"
                                  onKeyDown={handleNumberInputKeyDown}
                                />
                              ...
                              <TableCell className="py-1">
                                <Input
                                  ...
                                  value={
                                    quantidadeR3 === 0
                                      ? ""
                                      : quantidadeR3.toString()
                                  }
                                  onChange={(e) =>
                                    setQuantidadeR3(Number(e.target.value))
                                  }
                                  placeholder="0"
                                  onKeyDown={handleNumberInputKeyDown}
                                />
                      ...
                      <div className="flex justify-between items-center p-3 bg-background rounded-lg border">
                        <span className="font-bold text-sm">
                          TOTAL DE RAÇÕES OPERACIONAIS
                        </span>
                        <span className="font-extrabold text-lg text-secondary">
                          {formatNumber(totalRacoesOperacionais, 0)} un.
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-2">
                        * Nota: O valor monetário desta solicitação é considerado
                        R$ 0,00 para fins de cálculo logístico interno.
                      </p>
                      <div className="flex justify-end gap-2 pt-4">
                        <Button
                          type="button"
              ...
              <div className="space-y-4 border-b pb-4 pt-4">
                ...
                {isConsolidationOutdated && (
                  <Alert variant="destructive">
                    ...
                    <AlertDescription className="font-medium">
                      Atenção: A configuração da OM foi alterada. Clique em
                      "Salvar Item da Categoria" na(s) aba(s) ativa(s) para
                      atualizar os cálculos antes de salvar os registros.
                    </AlertDescription>
                  </Alert>
                )}
                      ...
                      <div className="space-y-1 text-sm">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          <div className="font-medium text-muted-foreground">
                            Efetivo / Dias / Ref. Int.
                          </div>
                          <div className="font-semibold text-foreground text-right">
                            {formatNumber(RACAO_QUENTE_DATA.efetivo, 0)} mil. /{" "}
                            {RACAO_QUENTE_DATA.dias_operacao} dias /{" "}
                            {RACAO_QUENTE_DATA.nr_ref_int} ref.
                          </div>
                          <div className="font-medium text-muted-foreground">
                            Fase da Atividade
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          ...
                          <div className="font-bold text-blue-600 text-right">
                            Total: {formatCurrency(RACAO_QUENTE_DATA.total_qs || 0)}
                          </div>
                          <div className="col-span-2 flex justify-between items-center">
                            <span className="text-muted-foreground pl-2">
                              Complemento de Etapa
                            </span>
                            <span className="text-[10px] text-muted-foreground/80">
                              {formatFormula(
                                RACAO_QUENTE_DATA.efetivo,
                                RACAO_QUENTE_DATA.dias_operacao,
                                RACAO_QUENTE_DATA.nr_ref_int || 0,
                                RACAO_QUENTE_DATA.valor_qs || 0,
                                0,
                                "complemento",
                                RACAO_QUENTE_DATA.complemento_qs || 0
                              )}
                            </span>
                          </div>
                          <div className="col-span-2 flex justify-between items-center">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          ...
                          <div className="font-bold text-green-600 text-right">
                            Total: {formatCurrency(RACAO_QUENTE_DATA.total_qr || 0)}
                          </div>
                          <div className="col-span-2 flex justify-between items-center">
                            <span className="text-muted-foreground pl-2">
                              Complemento de Etapa
                            ...
                            <span className="text-[10px] text-muted-foreground/80">
                              {formatFormula(
                                ...
                                RACAO_QUENTE_DATA.dias_operacao,
                                RACAO_QUENTE_DATA.nr_ref_int || 0,
                                RACAO_QUENTE_DATA.valor_qr || 0,
                                0,
                                "complemento",
                                RACAO_QUENTE_DATA.complemento_qr || 0
                              )}
                            </span>
                          </div>
                          <div className="col-span-2 flex justify-between items-center">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 pt-2 text-xs">
                          ...
                          <div className="flex justify-between">
                            <span className="font-medium text-blue-600">
                              QS: {RACAO_QUENTE_DATA.om_qs} (
                              {formatCodug(RACAO_QUENTE_DATA.ug_qs)})
                            </span>
                            <span className="font-medium text-green-600">
                              QR: {organizacao} ({formatCodug(ug)})
                            </span>
                          </div>
                          <div className="font-medium text-muted-foreground">
                            ND 33.90.30 (Material)
                          </div>
                          <div className="flex justify-between">
                            <span className="font-medium text-blue-600">
                              QS: {formatCurrency(RACAO_QUENTE_DATA.total_qs || 0)}
                            </span>
                            <span className="font-medium text-green-600">
                              QR: {formatCurrency(RACAO_QUENTE_DATA.total_qr || 0)}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <Card className="p-4 bg-secondary/10 border-secondary">
                      ...
                      <div className="space-y-1 text-sm">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          ...
                          <div className="font-semibold text-foreground text-right">
                            {formatNumber(
                              currentOMConsolidatedData.RACAO_OPERACIONAL.efetivo,
                              0
                            )}{" "}
                            mil. /{" "}
                            {
                  ...
                  {totalUnidadesConsolidado > 0 && (
                    <div className="flex justify-between items-center text-xs pt-1">
                      ...
                      <span className="font-semibold text-secondary">
                        {formatNumber(totalUnidadesConsolidado, 0)} un.
                      </span>
                    </div>
                  )}
                </div>
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <Button
                    ...
                    disabled={
                      loading || !isConfigReady || isConsolidationOutdated
                    }
                  >
                    {loading
                      ? "Aguarde..."
            ...
            {registros.length > 0 && (
              <div className="space-y-4 mt-8">
                ...
                <div className="space-y-4">
                  {Object.entries(registrosAgrupadosPorOM).map(
                    ([omKey, omRegistros]) => {
                      const totalMonetarioOM = omRegistros
                        .filter((r) => r.categoria === "RACAO_QUENTE")
                        .reduce(
                          (sum, r) =>
                            sum + r.calculos.totalQS + r.calculos.totalQR,
                          0
                        );
                      const totalUnidadesOM = omRegistros
                        ...
                        .reduce(
                          (sum, r) =>
                            sum + (r.quantidadeR2 || 0) + (r.quantidadeR3 || 0),
                          0
                        );
                            ...
                            <div className="flex flex-col items-end">
                              ...
                              {totalUnidadesOM > 0 && (
                                <span className="font-extrabold text-sm text-secondary">
                                  {formatNumber(totalUnidadesOM, 0)} un. (R2/R3)
                                </span>
                              )}
                            </div>
                          </div>
                          <div className="space-y-3">
                            {omRegistros.map((registro) => {
                              ...
                              const totalGeralRegistro = isRacaoQuente
                                ? registro.calculos.totalQS +
                                  registro.calculos.totalQR
                                : (registro.quantidadeR2 || 0) +
                                  (registro.quantidadeR3 || 0);
                              const fases = formatFasesParaTexto(
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <span className="font-bold text-lg text-primary/80 whitespace-nowrap">
                                        {isRacaoQuente
                                          ? formatCurrency(totalGeralRegistro)
                                          : `${formatNumber(
                                              totalGeralRegistro,
                                              0
                                            )} un.`}
              ...
              <div className="space-y-4 mt-6">
                ...
                {registros.map((registro) => {
                  ...
                  const hasCustomMemoria = isRacaoQuente
                    ? !!(
                        registro.memoriaQSCustomizada ||
                        registro.memoriaQRCustomizada
                      )
                  ...
                  if (isRacaoQuente) {
                    const { qs, qr } =
                      generateRacaoQuenteMemoriaCalculo(registro);
                    memoriaQSFinal = isEditing
                      ? memoriaQSEdit
                      : registro.memoriaQSCustomizada || qs;
                    memoriaQRFinal = isEditing
                      ? memoriaQREdit
                      : registro.memoriaQRCustomizada || qr;
                  } else {
                    const op = generateRacaoOperacionalMemoriaCalculo(registro);
                    memoriaOpFinal = isEditing
                      ? memoriaOpEdit
                      : registro.memoria_calculo_op_customizada || op;
                  }
                  return (
                    <Card key={registro.id} className="p-6 bg-muted/30">
                          ...
                          {!isEditing ? (
                            <>
                              <Button
                                ...
                                variant="outline"
                                onClick={() =>
                                  handleIniciarEdicaoMemoria(registro)
                                }
                                disabled={loading}
                                className="gap-2"
                              >
                            ...
                            <>
                              <Button
                                ...
                                variant="default"
                                onClick={() =>
                                  handleSalvarMemoriaCustomizada(registro.id)
                                }
                                disabled={loading}
                                className="gap-2"
                              >
                      ...
                      {isRacaoQuente ? (
                        <>
                          <div className="space-y-2 mb-6">
                            <h5 className="font-bold text-sm text-blue-600">
                              QS - Quantitativo de Subsistência (RM:{" "}
                              {registro.omQS})
                            </h5>
                            <Card className="p-4 bg-background rounded-lg border">
                              <Textarea
                                ...
                                onChange={(e) =>
                                  isEditing && setMemoriaQSEdit(e.target.value)
                                }
                                readOnly={!isEditing}
                                rows={10}
                                className={cn(
                          ...
                          <div className="space-y-2">
                            ...
                            <Card className="p-4 bg-background rounded-lg border">
                              <Textarea
                                ...
                                onChange={(e) =>
                                  isEditing && setMemoriaQREdit(e.target.value)
                                }
                                readOnly={!isEditing}
                                rows={10}
                                className={cn(
                        ...
                        <div className="space-y-2">
                          <h5 className="font-bold text-sm text-secondary">
                            Ração Operacional (R2/R3)
                          </h5>
                          <Card className="p-4 bg-background rounded-lg border">
                            <Textarea
                              ...
                              onChange={(e) =>
                                isEditing && setMemoriaOpEdit(e.target.value)
                              }
                              readOnly={!isEditing}
                              rows={8}
                              className={cn(
                                "font-mono text-xs whitespace-pre-wrap text-foreground",
                                isEditing &&
                                  "border-secondary focus:ring-2 focus:ring-secondary"
                              )}
                            />
                          </Card>
                        </div>
