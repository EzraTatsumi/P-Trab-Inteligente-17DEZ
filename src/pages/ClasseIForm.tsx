    ...
    try {
      ...
      const registrosCarregados: ClasseIRegistro[] = (data || []).map((r) => {
        ...
        return {
          ...
          calculos: {
            // CORREÇÃO: Força o uso da nova regra de cálculo sobrepondo o dado antigo do banco
            totalQS: derivedCalculations.totalQS,
            totalQR: derivedCalculations.totalQR,
            ...
            complementoQS: derivedCalculations.complementoQS,
            etapaQS: derivedCalculations.etapaQS,
            complementoQR: derivedCalculations.complementoQR,
            etapaQR: derivedCalculations.etapaQR,
          },
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-muted/20">
      <div className="container mx-auto px-4 py-2">
        ...
        <Card className="p-6 backdrop-blur-sm bg-card/95 border-primary/10 max-w-6xl mx-auto">
          <div className="mb-4">
            <h1 className="text-3xl font-bold mb-2 bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">Classe I - Subsistência</h1>
            <p className="text-muted-foreground">Configure a sua necessidade de alimentação (Ração Quente e Ração Operacional) por OM.</p>
          </div>
          <div className="space-y-6">
              ...
              <div className="space-y-6 p-4 bg-white rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <OmSelector selectedOmId={selectedOmId} onChange={handleOMChange} placeholder="Selecione uma OM de Destino..." initialOmName={initialOmName} initialOmUg={initialOmUg} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="ug">UG de Destino</Label>
                    <Input id="ug" value={formatCodug(ug)} readOnly disabled={true} className="disabled:opacity-60" />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="efetivo">Efetivo de Militares *</Label>
                    <Input id="efetivo" type="number" min="1" className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={efetivo === 0 ? "" : efetivo.toString()} onChange={(e) => setEfetivo(Number(e.target.value))} placeholder="Ex: 246" onKeyDown={handleNumberInputKeyDown} disabled={!organizacao} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="diasOperacao">Dias de Atividade *</Label>
                    <Input id="diasOperacao" type="number" min="1" className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={diasOperacao === 0 ? "" : diasOperacao.toString()} onChange={(e) => setDiasOperacao(Number(e.target.value))} placeholder="Ex: 30" onKeyDown={handleNumberInputKeyDown} disabled={!organizacao} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="faseAtividade">Fase da Atividade *</Label>
                    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                      <PopoverTrigger asChild>
                        <Button variant="outline" role="combobox" type="button" className="w-full justify-between" disabled={!organizacao}>
                          <span className="truncate">{displayFases || "Selecione a(s) fase(s)..."}</span>
                        ...
                        <Command>
                          <CommandGroup>
                            {FASES_PADRAO.map((fase) => (
                              <CommandItem key={fase} value={fase} onSelect={() => handleFaseChange(fase, !fasesAtividade.includes(fase))} className="flex items-center justify-between cursor-pointer">
                                <div className="flex items-center space-x-2">
                                  <Checkbox checked={fasesAtividade.includes(fase)} onCheckedChange={(checked) => handleFaseChange(fase, !!checked)} />
                                  <Label>{fase}</Label>
                                </div>
                          ...
                          <div className="p-2 border-t">
                            ...
                            <Input value={customFaseAtividade} onChange={(e) => setCustomFaseAtividade(e.target.value)} placeholder="Ex: Patrulhamento" onKeyDown={handleEnterToNextField} />
                          </div>
                        </Command>
                      </PopoverContent>
                    </Popover>
                ...
                <Tabs value={selectedTab} onValueChange={(value) => setSelectedTab(value as CategoriaClasseI)}>
                    ...
                    <div className="space-y-2 p-4 bg-muted/50 rounded-lg">
                      ...
                      <div className="grid md:grid-cols-3 gap-6 pt-2">
                        <div className="space-y-2">
                          ...
                          <Select value={nrRefInt.toString()} onValueChange={(value) => setNrRefInt(Number(value))}>
                        ...
                        <div className="space-y-2">
                          ...
                          <RmSelector value={omQS} onChange={handleRMQSChange} placeholder="Selecione a RM de destino do QS..." disabled={!organizacao} />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="ugQS">UG de Destino</Label>
                          <Input id="ugQS" value={formatCodug(ugQS)} readOnly disabled={true} className="disabled:opacity-60" />
                        </div>
                      </div>
                      <div className="grid md:grid-cols-2 gap-4 pt-4">
                        <div className="space-y-2">
                          <Label>Valor QS (Diretriz {diretrizAno || 'Atual'})</Label>
                          <Input value={formatCurrency(valorQS)} readOnly disabled className="font-semibold text-blue-600 disabled:opacity-100" />
                        </div>
                        <div className="space-y-2">
                          <Label>Valor QR (Diretriz {diretrizAno || 'Atual'})</Label>
                          <Input value={formatCurrency(valorQR)} readOnly disabled className="font-semibold text-green-600 disabled:opacity-100" />
                        </div>
                      </div>
                      {calculosRacaoQuente && (
                      ...
                      <div className="flex justify-end gap-2 pt-4">
                        <Button type="button" onClick={handleSaveCategoryConfig} className="gap-2" disabled={loading || !organizacao || diasOperacao <= 0 || efetivo <= 0 || (!displayFases)}>
                  ...
                  <TabsContent value="RACAO_OPERACIONAL" className="mt-4">
                    <div className="space-y-2 p-4 bg-white rounded-lg">
                      ...
                      <p className="text-sm text-muted-foreground">Informe a quantidade total de rações operacionais necessárias para o efetivo e dias de atividade.</p>
                        ...
                        <Table className="w-full">
                          ...
                          <TableBody>
                            <TableRow className="h-12">
                              <TableCell className="font-medium text-sm py-1">Ração Operacional R2 (24h)</TableCell>
                              <TableCell className="py-1">
                                <Input id="quantidadeR2" type="number" min="0" className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none h-8 text-center" value={quantidadeR2 === 0 ? "" : quantidadeR2.toString()} onChange={(e) => setQuantidadeR2(Number(e.target.value))} placeholder="0" onKeyDown={handleNumberInputKeyDown} />
                              </TableCell>
                            </TableRow>
                            <TableRow className="h-12">
                              <TableCell className="font-medium text-sm py-1">Ração Operacional R3 (12h)</TableCell>
                              <TableCell className="py-1">
                                <Input id="quantidadeR3" type="number" min="0" className="[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none h-8 text-center" value={quantidadeR3 === 0 ? "" : quantidadeR3.toString()} onChange={(e) => setQuantidadeR3(Number(e.target.value))} placeholder="0" onKeyDown={handleNumberInputKeyDown} />
                              </TableCell>
                            </TableRow>
                          </TableBody>
                        </Table>
                      ...
                      <div className="flex justify-between items-center p-3 bg-background rounded-lg border">
                        ...
                        <span className="font-extrabold text-lg text-secondary">{formatNumber(totalRacoesOperacionais, 0)} un.</span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-2">* Nota: O valor monetário desta solicitação é considerado R$ 0,00 para fins de cálculo logístico interno.</p>
                      <div className="flex justify-end gap-2 pt-4">
                        <Button type="button" onClick={handleSaveCategoryConfig} className="gap-2" disabled={loading || !organizacao || diasOperacao <= 0 || efetivo <= 0 || (!displayFases)}>
                          <Check className="h-4 w-4" />
                          {editingRegistroId ? "Atualizar Item da Categoria" : "Salvar Item da Categoria"}
              ...
              <div className="space-y-4 border-b pb-4 pt-4">
                ...
                {isConsolidationOutdated && (
                  <Alert variant="destructive">
                    ...
                    <AlertDescription className="font-medium">Atenção: A configuração da OM foi alterada. Clique em "Salvar Item da Categoria" na(s) aba(s) ativa(s) para atualizar os cálculos antes de salvar os registros.</AlertDescription>
                  </Alert>
                )}
                <div className="space-y-4">
                  {RACAO_QUENTE_DATA && (
                    <Card className="p-4 bg-secondary/10 border-secondary">
                      <div className="flex items-center justify-between mb-3 border-b pb-2">
                        <h4 className="font-bold text-base text-primary flex items-center gap-2">Ração Quente (QS/QR)</h4>
                        <span className="font-extrabold text-lg text-primary">{formatCurrency(RACAO_QUENTE_DATA.total_geral)}</span>
                      </div>
                      <div className="space-y-1 text-sm">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          <div className="font-medium text-muted-foreground">Efetivo / Dias / Ref. Int.</div>
                          <div className="font-semibold text-foreground text-right">{formatNumber(RACAO_QUENTE_DATA.efetivo)} mil. / {RACAO_QUENTE_DATA.dias_operacao} dias / {RACAO_QUENTE_DATA.nr_ref_int} ref.</div>
                          <div className="font-medium text-muted-foreground">Fase da Atividade</div>
                          <div className="font-semibold text-foreground text-right">{formatFasesParaTexto(RACAO_QUENTE_DATA.fase_atividade)}</div>
                        </div>
                        <div className="h-px bg-border my-2" />
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          ...
                          <div className="col-span-2 flex justify-between items-center">
                            ...
                            <span className="text-[10px] text-muted-foreground/80">{formatFormula(RACAO_QUENTE_DATA.efetivo, RACAO_QUENTE_DATA.dias_operacao, RACAO_QUENTE_DATA.nr_ref_int || 0, RACAO_QUENTE_DATA.valor_qs || 0, 0, 'complemento', RACAO_QUENTE_DATA.complemento_qs || 0)}</span>
                          </div>
                          <div className="col-span-2 flex justify-between items-center">
                            <span className="text-muted-foreground pl-2">Etapa a Solicitar</span>
                            <span className="text-[10px] text-muted-foreground/80">{formatFormula(RACAO_QUENTE_DATA.efetivo, RACAO_QUENTE_DATA.dias_operacao, RACAO_QUENTE_DATA.nr_ref_int || 0, RACAO_QUENTE_DATA.valor_qs || 0, diasEtapaSolicitada, 'etapa', RACAO_QUENTE_DATA.etapa_qs || 0)}</span>
                          </div>
                        </div>
                        <div className="h-px bg-border my-2" />
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          ...
                          <div className="col-span-2 flex justify-between items-center">
                            <span className="text-muted-foreground pl-2">Complemento de Etapa</span>
                            <span className="text-[10px] text-muted-foreground/80">{formatFormula(RACAO_QUENTE_DATA.efetivo, RACAO_QUENTE_DATA.dias_operacao, RACAO_QUENTE_DATA.nr_ref_int || 0, RACAO_QUENTE_DATA.valor_qr || 0, 0, 'complemento', RACAO_QUENTE_DATA.complemento_qr || 0)}</span>
                          </div>
                          <div className="col-span-2 flex justify-between items-center">
                            <span className="text-muted-foreground pl-2">Etapa a Solicitar</span>
                            <span className="text-[10px] text-muted-foreground/80">{formatFormula(RACAO_QUENTE_DATA.efetivo, RACAO_QUENTE_DATA.dias_operacao, RACAO_QUENTE_DATA.nr_ref_int || 0, RACAO_QUENTE_DATA.valor_qr || 0, diasEtapaSolicitada, 'etapa', RACAO_QUENTE_DATA.etapa_qr || 0)}</span>
                          </div>
                        </div>
                        <div className="h-px bg-border my-2" />
                  ...
                  {currentOMConsolidatedData?.RACAO_OPERACIONAL && (
                    <Card className="p-4 bg-secondary/10 border-secondary">
                      <div className="flex items-center justify-between mb-3 border-b pb-2">
                        <h4 className="font-bold text-base text-primary flex items-center gap-2">Ração Operacional (R2/R3)</h4>
                        <span className="font-extrabold text-lg text-secondary">{formatNumber(currentOMConsolidatedData.RACAO_OPERACIONAL.total_unidades, 0)} un.</span>
                      </div>
                      <div className="space-y-1 text-sm">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                          <div className="font-medium text-muted-foreground">Efetivo / Dias</div>
                          <div className="font-semibold text-foreground text-right">{formatNumber(currentOMConsolidatedData.RACAO_OPERACIONAL.efetivo)} mil. / {currentOMConsolidatedData.RACAO_OPERACIONAL.dias_operacao} dias</div>
                          <div className="font-medium text-muted-foreground">Fase da Atividade</div>
                          <div className="font-semibold text-foreground text-right">{formatFasesParaTexto(currentOMConsolidatedData.RACAO_OPERACIONAL.fase_atividade)}</div>
                        </div>
                        <div className="h-px bg-border my-2" />
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 pt-2 text-xs">
                          <div className="font-medium text-muted-foreground">Ração Operacional R2 (24h)</div>
                          <div className="font-semibold text-secondary text-right">{formatNumber(currentOMConsolidatedData.RACAO_OPERACIONAL.quantidade_r2 || 0, 0)} un.</div>
                          <div className="font-medium text-muted-foreground">Ração Operacional R3 (12h)</div>
                          <div className="font-semibold text-secondary text-right">{formatNumber(currentOMConsolidatedData.RACAO_OPERACIONAL.quantidade_r3 || 0, 0)} un.</div>
                        </div>
                      </div>
                    </Card>
                  )}
                ...
                <div className="flex flex-col p-3 bg-primary/10 rounded-lg border border-primary/20 mt-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-extrabold text-xl text-primary">{formatCurrency(totalMonetarioConsolidado)}</span>
                  </div>
                  {totalUnidadesConsolidado > 0 && (
                    <div className="flex justify-between items-center text-xs pt-1">
                      <span className="text-muted-foreground">Total de Rações Operacionais</span>
                      <span className="font-semibold text-secondary">{formatNumber(totalUnidadesConsolidado, 0)} un.</span>
                    </div>
                  )}
                </div>
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <Button type="button" onClick={handleFinalSave} disabled={loading || !isConfigReady || isConsolidationOutdated}>
              ...
              <div className="space-y-4 mt-8">
                ...
                <div className="space-y-4">
                  {Object.entries(registrosAgrupadosPorOM).map(([omKey, omRegistros]) => {
                    const totalMonetarioOM = omRegistros.filter(r => r.categoria === 'RACAO_QUENTE').reduce((sum, r) => sum + r.calculos.totalQS + r.calculos.totalQR, 0);
                    const totalUnidadesOM = omRegistros.filter(r => r.categoria === 'RACAO_OPERACIONAL').reduce((sum, r) => sum + (r.quantidadeR2 || 0) + (r.quantidadeR3 || 0), 0);
                    const omName = omKey.split(' (')[0];
                    const ugFormatted = omKey.split(' (')[1].replace(')', '');
                    return (
                      <Card key={omKey} className="p-4 bg-primary/5 border-primary/20">
                        <div className="flex items-center justify-between mb-3 border-b pb-2">
                          <h3 className="font-bold text-lg text-primary">{omName} (UG: {ugFormatted})</h3>
                          <div className="flex flex-col items-end">
                            {totalMonetarioOM > 0 && (
                              <span className="font-extrabold text-xl text-primary">{formatCurrency(totalMonetarioOM)}</span>
                            )}
                            {totalUnidadesOM > 0 && (
                              <span className="font-extrabold text-sm text-secondary">{formatNumber(totalUnidadesOM, 0)} un. (R2/R3)</span>
                            )}
                          </div>
                        </div>
                        <div className="space-y-3">
                          {omRegistros.map((registro) => {
                            ...
                            const totalGeralRegistro = isRacaoQuente ? (registro.calculos.totalQS + registro.calculos.totalQR) : ((registro.quantidadeR2 || 0) + (registro.quantidadeR3 || 0));
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex flex-col">
                                    <div className="flex items-center gap-2">
                                      <h4 className="font-semibold text-base text-foreground">{isRacaoQuente ? 'Ração Quente (R1)' : 'Ração Operacional (R2/R3)'}</h4>
                                      <Badge variant="outline" className="text-xs">{fases}</Badge>
                                    </div>
                                    <p className="text-xs text-muted-foreground">Efetivo: {registro.efetivo} | Dias: {registro.diasOperacao}</p>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className="font-bold text-lg text-primary/80 whitespace-nowrap">{isRacaoQuente ? formatCurrency(totalGeralRegistro) : `${formatNumber(totalGeralRegistro, 0)} un.`}</span>
                                    <div className="flex gap-1">
                                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEditRegistro(registro)}>
                                        <Pencil className="h-4 w-4" />
                                      </Button>
                                      <Button variant="ghost" size="icon" onClick={() => handleRemover(registro.id)} className="h-8 w-8 text-destructive hover:bg-destructive/10">
                                ...
                                {isRacaoQuente ? (
                                  ...
                                  <div className="pt-2 border-t mt-2 space-y-1 text-xs">
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-medium text-secondary">{formatNumber(registro.quantidadeR2 || 0, 0)} un.</span>
                                    </div>
                                    <div className="flex justify-between">
                                      <span className="text-muted-foreground">R3 (12h):</span>
                                      <span className="font-medium text-secondary">{formatNumber(registro.quantidadeR3 || 0, 0)} un.</span>
                                    </div>
                                  </div>
                                )}
                              </Card>
            ...
            {registros.length > 0 && (
              <div className="space-y-4 mt-6">
                <h3 className="text-xl font-bold flex items-center gap-2">📋 Memórias de Cálculo Detalhadas</h3>
                {registros.map((registro) => {
                  const isRacaoQuente = registro.categoria === 'RACAO_QUENTE';
                  const isEditing = editingMemoriaId === registro.id;
                  const hasCustomMemoria = isRacaoQuente ? !!(registro.memoriaQSCustomizada || registro.memoriaQRCustomizada) : !!registro.memoria_calculo_op_customizada;
                  let memoriaQSFinal = "";
                  let memoriaQRFinal = "";
                  let memoriaOpFinal = "";
                  if (isRacaoQuente) {
                  ...
                  return (
                    <Card key={registro.id} className="p-6 bg-muted/30">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <h4 className="text-lg font-semibold text-foreground">{registro.organizacao} (UG: {formatCodug(registro.ug)})</h4>
                          <Badge variant="default" className={cn(isRacaoQuente ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground")}>{isRacaoQuente ? 'Ração Quente (R1)' : 'Ração Operacional (R2/R3)'}</Badge>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {!isEditing ? (
                            <>
                              <Button size="sm" variant="outline" onClick={() => handleIniciarEdicaoMemoria(registro)} disabled={loading} className="gap-2">
                                <Pencil className="h-4 w-4" />
                                Editar Memória
                              </Button>
                              {hasCustomMemoria && (
                                <Button size="sm" variant="ghost" onClick={() => handleRestaurarMemoriaAutomatica(registro.id)} disabled={loading} className="gap-2 text-muted-foreground">
                            ...
                            <>
                              <Button size="sm" variant="default" onClick={() => handleSalvarMemoriaCustomizada(registro.id)} disabled={loading} className="gap-2">
                                <Check className="h-4 w-4" />
                                Salvar
                              </Button>
                              <Button size="sm" variant="outline" onClick={handleCancelarEdicaoMemoria} disabled={loading} className="gap-2">
                                <XCircle className="h-4 w-4" />
                                Cancelar
                              </Button>
                            </>
                      ...
                      {isRacaoQuente ? (
                        <>
                          <div className="space-y-2 mb-6">
                            ...
                            <Card className="p-4 bg-background rounded-lg border">
                              <Textarea value={memoriaQSFinal} onChange={(e) => isEditing && setMemoriaQSEdit(e.target.value)} readOnly={!isEditing} rows={10} className={cn("font-mono text-xs whitespace-pre-wrap text-foreground", isEditing && "border-blue-500 focus:ring-2 focus:ring-blue-500")} />
                            </Card>
                          </div>
                          <div className="space-y-2">
                            <h5 className="font-bold text-sm text-green-600">QR - Quantitativo de Reforço (OM: {registro.organizacao})</h5>
                            <Card className="p-4 bg-background rounded-lg border">
                              <Textarea value={memoriaQRFinal} onChange={(e) => isEditing && setMemoriaQREdit(e.target.value)} readOnly={!isEditing} rows={10} className={cn("font-mono text-xs whitespace-pre-wrap text-foreground", isEditing && "border-green-500 focus:ring-2 focus:ring-green-500")} />
                            </Card>
                          </div>
                        </>
                      ) : (
                        <div className="space-y-2">
                          ...
                          <Card className="p-4 bg-background rounded-lg border">
                            <Textarea value={memoriaOpFinal} onChange={(e) => isEditing && setMemoriaOpEdit(e.target.value)} readOnly={!isEditing} rows={8} className={cn("font-mono text-xs whitespace-pre-wrap text-foreground", isEditing && "border-secondary focus:ring-2 focus:ring-secondary")} />
                          </Card>
                        </div>
                      )}
                    </Card>
