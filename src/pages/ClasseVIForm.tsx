...
const ClasseVIForm = () => {
  ...
  const fetchRegistros = async () => {
    ...
    (data || []).forEach(r => {
      ...
      const record = {
        ...r,
        // CORRIGIDO: Casting mais seguro para JSONB
        itens_equipamentos: (r.itens_equipamentos || []) as any as ItemClasseVI[],
  ...
  const handleSalvarRegistros = async () => {
    ...
    if (!form.organizacao || !form.ug) {
      toast.error("Selecione uma OM detentora");
      return;
    }
    if (form.dias_operacao <= 0) {
      toast.error("Dias de operação deve ser maior que zero");
      return;
    }
  ...
  const registrosAgrupadosPorOM = useMemo(() => {
    return registros.reduce((acc, registro) => {
      ...
      const key = `${omDetentora} (UG: ${formatCodug(ugDetentora)})`;
                                    ...
                                    <Button
                                      ...
                                      onClick={async () => {
                                        if (confirm(`Deseja realmente deletar o registro de Classe VI para ${omName} (${registro.categoria})?`)) {
                                          try {
                                            const { error } = await supabase.from("classe_vi_registros")
                                              .delete()
                                              .eq("id", registro.id);
