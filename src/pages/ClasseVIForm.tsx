...
const generateClasseIIMemoriaCalculo = (registro: ClasseIIRegistro): string => {
  ...
  const memoria = `33.90.30/33.90.39 - Aquisição de Material de Engenharia (Classe VI) para ${organizacao} (UG: ${ug}), durante ${dias_operacao} dias de ${faseFormatada}.
...
export default function ClasseVIForm() {
  ...
  const loadDiretrizes = async () => {
    try {
      ...
      if (!anoReferencia) {
        setDiretrizAno(null);
        setDiretrizesDisponiveis(defaultClasseVIConfig as DiretrizClasseII[]);
        return;
      }
      
      setDiretrizAno(anoReferencia);

      // 3. Buscar diretrizes de Classe VI
      const { data, error } = await supabase
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <Card ref={formRef}>
          <CardHeader>
            <CardTitle>Classe VI - Material de Engenharia</CardTitle>
            <CardDescription>
              Configure a necessidade de material de Engenharia por OM.
            </CardDescription>
          </CardHeader>
