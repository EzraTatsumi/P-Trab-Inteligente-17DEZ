...
import { defaultClasseIIIConfig } from "@/data/classeIIIData"; // Assumindo que existe

type Categoria = 'Combustível' | 'Lubrificante' | 'Embarcação' | 'Equipamento de Engenharia'; // Incluindo categorias de Classe VI para padronização
...
const initialCategoryAllocations: Record<Categoria, CategoryAllocation> = {
    'Combustível': { total_valor: 0, nd_39_input: "", nd_30_value: 0, nd_39_value: 0, om_destino_recurso: "", ug_destino_recurso: "", selectedOmDestinoId: undefined },
    'Lubrificante': { total_valor: 0, nd_39_input: "", nd_30_value: 0, nd_39_value: 0, om_destino_recurso: "", ug_destino_recurso: "", selectedOmDestinoId: undefined },
...
const generateDetalhamento = (itens: ItemClasseIII[], diasOperacao: number, organizacao: string, ug: string, faseAtividade: string, omDestino: string, ugDestino: string, valorND30: number, valorND39: number): string => {
    ...
    return `33.90.30 / 33.90.39 - Aquisição de Material de Classe III (Diversos) para ${totalItens} itens, durante ${diasOperacao} dias de ${faseFormatada}, para ${organizacao}.
...
const ClasseIIIForm = () => {
  ...
  const loadDiretrizes = async () => {
    try {
      ...
      if (!anoReferencia) {
        ...
        setDiretrizes(defaultClasseIIIConfig as DiretrizClasseII[]);
        return;
      }

      const { data: classeIIIData, error } = await supabase
        .from("diretrizes_classe_ii")
        .select("*")
        .eq("user_id", user.id)
        .eq("ano_referencia", anoReferencia)
        .eq("ativo", true)
        .in("categoria", CATEGORIAS); // Filtrar apenas categorias da Classe III

      if (error) throw error;

      if (classeIIIData && classeIIIData.length > 0) {
        setDiretrizes((classeIIIData || []) as DiretrizClasseII[]);
      } else {
        setDiretrizes(defaultClasseIIIConfig as DiretrizClasseII[]);
        toast.warning(`Itens de Classe III não configurados para o ano ${anoReferencia}. Usando valores padrão.`);
      }
  ...
  const fetchRegistros = async () => {
    ...
    const uniqueRecordsMap = new Map<string, ClasseIIIRegistro>();
    (data || []).forEach(r => {
        const key = `${r.organizacao}-${r.ug}-${r.categoria}`;
        const record = {
            ...r,
            itens_equipamentos: (r.itens_equipamentos || []) as ItemClasseIII[],
  ...
  const handleSalvarRegistros = async () => {
    ...
    try {
      ...
      toast.success(editingId ? "Registros de Classe III atualizados com sucesso!" : "Registros de Classe III salvos com sucesso!");
  ...
  const handleEditarRegistro = async (registro: ClasseIIIRegistro) => {
    ...
    (allRecords || []).forEach(r => {
        ...
        const items = (r.itens_equipamentos || []) as ItemClasseIII[];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Solicitação de recursos para aquisição de combustíveis e lubrificantes.
            </CardDescription>
          </CardHeader>
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex flex-col">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {/* REMOVIDO: Badge da Categoria para padronizar com V, VI, VII */}
                                                    </div>
                                                    <p className="text-xs text-muted-foreground">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="flex gap-1">
                                                        ...
                                                        <Button
                                                            ...
                                                            size="icon"
                                                            onClick={() => {
                                                                if (confirm(`Deseja realmente deletar o registro de Classe III para ${omName} (${registro.categoria})?`)) {
