...
import { ArrowLeft, Loader2, FileText, Package, Utensils, Briefcase, HardHat, Plane, ClipboardList, Frown, Download, Printer, FileSpreadsheet } from "lucide-react";
...
const PTrabReportManager = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const currentNomeRM = currentPtrab.rm_vinculacao || '';
  ...
  const gruposPorOM = useMemo(() => {
    ...
    const currentNomeRM = ptrabData?.rm_vinculacao || '';
    ...
    return grupos;
  }, [registrosClasseI, registrosClasseII, registrosClasseIII, refLPC, ptrabData]);
  ...
  const omsOrdenadas = useMemo(() => {
    ...
    const currentNomeRM = nomeRM;
    
    return oms.sort((a, b) => {
        const aPriority = getOMPriority(a, currentNomeRM);
        const bPriority = getOMPriority(b, currentNomeRM);
  ...
  const calcularTotaisPorOM = useCallback((grupo: GrupoOM, nomeOM: string) => {
    ...
    const isRMFornecedora = isRegiaoMilitar(nomeOM, currentNomeRM);
