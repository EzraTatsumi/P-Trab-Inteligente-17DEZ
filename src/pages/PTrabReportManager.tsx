...
export const generateDiariaMemoriaCalculoUnificada = (registro: DiariaRegistro, diretrizesOp: Tables<'diretrizes_operacionais'> | null): string => {
    if (registro.detalhamento_customizado && registro.detalhamento_customizado.trim().length > 0) return registro.detalhamento_customizado;
    if (!diretrizesOp) return registro.detalhamento || "Diretrizes Operacionais ausentes. Memória de cálculo não gerada.";
...
interface NoDataFallbackProps { reportName: string; message: string; }
...
const PTrabReportManager = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (classeIIIData !== null) setRegistrosClasseIII((classeIIIData || []).map((r: any) => ({ ...r, itens_equipamentos: (r.itens_equipamentos as any as ItemClasseIII[] | null) || null })) as ClasseIIIRegistro[]);
      ...
      if (passagemData !== null) setRegistrosPassagem((passagemData || []).map((r: any) => ({ ...r, valor_unitario: Number(r.valor_unitario || 0), valor_total: Number(r.valor_total || 0), valor_nd_33: Number(r.valor_nd_33 || 0), quantidade_passagens: r.quantidade_passagens || 0, is_ida_volta: r.is_ida_volta || false, efetivo: r.efetivo || 0 })) as PassagemRegistro[]);
      if (concessionariaData !== null) setRegistrosConcessionaria((concessionariaData || []).map((r: any) => ({ ...r, valor_unitario: Number(r.valor_unitario || 0), consumo_pessoa_dia: Number(r.consumo_pessoa_dia || 0), valor_total: Number(r.valor_total || 0), valor_nd_39: Number(r.valor_nd_39 || 0), dias_operacao: r.dias_operacao || 0, efetivo: r.efetivo || 0, nome_concessionaria: '', unidade_custo: '', fonte_consumo: null, fonte_custo: null })) as ConcessionariaRegistro[]);
      if (materialConsumoData !== null) setRegistrosMaterialConsumo((materialConsumoData || []).map((r: any) => ({ ...r, valor_total: Number(r.valor_total || 0), valor_nd_30: Number(r.valor_nd_30 || 0), valor_nd_39: Number(r.valor_nd_39 || 0), dias_operacao: r.dias_operacao || 0, efetivo: r.efetivo || 0 })) as MaterialConsumoRegistro[]);
      if (complementoAlimentacaoData !== null) setRegistrosComplementoAlimentacao((complementoAlimentacaoData || []).map((r: any) => ({ ...r, valor_total: Number(r.valor_total || 0), valor_nd_30: Number(r.valor_nd_30 || 0), valor_nd_39: Number(r.valor_nd_39 || 0), dias_operacao: r.dias_operacao || 0, efetivo: r.efetivo || 0, itens_aquisicao: (r.itens_aquisicao as any) || [] })) as any);
      if (servicosTerceirosData !== null) setRegistrosServicosTerceiros((servicosTerceirosData || []).map((r: any) => ({ ...r, valor_total: Number(r.valor_total || 0), valor_nd_30: Number(r.valor_nd_30 || 0), valor_nd_39: Number(r.valor_nd_39 || 0), dias_operacao: r.dias_operacao || 0, efetivo: r.efetivo || 0 })) as ServicoTerceiroRegistro[]);
      if (materialPermanenteData !== null) setRegistrosMaterialPermanente((materialPermanenteData || []).map((r: any) => ({ ...r, valor_total: Number(r.valor_total || 0), valor_nd_52: Number(r.valor_nd_52 || 0), dias_operacao: r.dias_operacao || 0, efetivo: r.efetivo || 0 })) as MaterialPermanenteRegistro[]);
      if (horasVooData !== null) setRegistrosHorasVoo((horasVooData || []).map((r: any) => ({ ...r, quantidade_hv: Number(r.quantidade_hv || 0), valor_nd_30: Number(r.valor_nd_30 || 0), valor_nd_39: Number(r.valor_nd_39 || 0), valor_total: Number(r.valor_total || 0), dias_operacao: r.dias_operacao || 0 })) as HorasVooRegistro[]);
      if (dorData !== null) {
          setRegistrosDOR(dorData || []);
          if (dorData && (dorData as any[]).length > 0) setSelectedDorId((prev) => prev ? prev : (dorData as any[])[0].id);
      }
      ...
      setTimeout(() => {
        runMission06(user.id, () => {
          ...
          if (!completed.includes(6)) {
            localStorage.setItem(`completed_missions_${user.id}`, JSON.stringify([...completed, 6]));
          }
  ...
  const gruposPorOM = useMemo(() => {
    ...
    const initializeGroup = (name: string) => {
        if (!grupos[name]) grupos[name] = { linhasQS: [], linhasQR: [], linhasClasseII: [], linhasClasseV: [], linhasClasseVI: [], linhasClasseVII: [], linhasClasseVIII: [], linhasClasseIX: [], linhasClasseIII: [], linhasConcessionaria: [] };
    };
    ...
    registrosClasseII.forEach((registro) => {
        ...
        if (CLASSE_V_CATEGORIES.includes(registro.categoria)) omGroup.linhasClasseV.push(linha);
        else if (CLASSE_VI_CATEGORIES.includes(registro.categoria)) omGroup.linhasClasseVI.push(linha);
        else if (CLASSE_VII_CATEGORIES.includes(registro.categoria)) omGroup.linhasClasseVII.push(linha);
        else if (CLASSE_VIII_CATEGORIES.includes(registro.categoria)) omGroup.linhasClasseVIII.push(linha);
        else if (CLASSE_IX_CATEGORIES.includes(registro.categoria)) omGroup.linhasClasseIX.push(linha);
        else omGroup.linhasClasseII.push(linha);
    });
    ...
    registrosClasseIII.forEach((registro) => {
        ...
        if (isCombustivel || isLubrificante) {
            let omDestinoRecurso = isCombustivel ? (registro.om_detentora || registro.organizacao) : registro.organizacao;
            ...
            itens.forEach(item => { if (!gruposGranulares[item.categoria]) gruposGranulares[item.categoria] = []; gruposGranulares[item.categoria].push(item); });
            Object.entries(gruposGranulares).forEach(([categoriaKey, itensGrupo]) => {
                ...
                let totalLitrosLinha = 0, valorTotalLinha = 0, precoLitroLinha = 0;
                itensGrupo.forEach(item => {
                    const itemWithInputs: ItemClasseIII = { ...item, preco_lubrificante_input: String(registro.preco_lubrificante || 0), consumo_lubrificante_input: String(registro.consumo_lubrificante_litro || 0) };
                    const totals = calculateItemTotals(itemWithInputs, refLPC, registro.dias_operacao);
                    if (isCombustivel) { totalLitrosLinha += totals.totalLitros; valorTotalLinha += totals.valorCombustivel; precoLitroLinha = totals.precoLitro; }
                    else if (isLubrificante) { totalLitrosLinha += totals.litrosLubrificante; valorTotalLinha += totals.valorLubrificante; }
                });
                if (valorTotalLinha === 0 && totalLitrosLinha === 0) return;
                const tipoSuprimento: LinhaClasseIII['tipo_suprimento'] = isCombustivel ? (primeiroItem.tipo_combustivel_fixo === 'GASOLINA' ? 'COMBUSTIVEL_GASOLINA' : 'COMBUSTIVEL_DIESEL') : 'LUBRIFICANTE';
                if (isLubrificante) precoLitroLinha = totalLitrosLinha > 0 ? valorTotalLinha / totalLitrosLinha : 0;
                let memoriaCalculo = "";
                const granularItem: GranularDisplayItem = { id: `${registro.id}-${categoriaKey}-${tipoSuprimento}`, om_destino: registro.organizacao, ug_destino: registro.ug, categoria: categoriaKey as any, suprimento_tipo: tipoSuprimento, valor_total: valorTotalLinha, total_litros: totalLitrosLinha, preco_litro: precoLitroLinha, dias_operacao: registro.dias_operacao, fase_atividade: registro.fase_atividade || '', valor_nd_30: valorTotalLinha, valor_nd_39: 0, original_registro: registro, detailed_items: itensGrupo.map(item => ({ ...item, preco_lubrificante_input: String(registro.preco_lubrificante || 0), consumo_lubrificante_input: String(registro.consumo_lubrificante_litro || 0) })) };
                const itemComMemoria = itensGrupo.find(i => !!i.memoria_customizada) || itensGrupo[0];
                if (itemComMemoria && itemComMemoria.memoria_customizada && itemComMemoria.memoria_customizada.trim().length > 0) memoriaCalculo = itemComMemoria.memoria_customizada;
                else memoriaCalculo = generateClasseIIIGranularUtility(granularItem as any, refLPC, isCombustivel ? (registro.om_detentora || '') : registro.organizacao, isCombustivel ? (registro.ug_detentora || '') : registro.ug);
                grupos[omDestinoRecurso].linhasClasseIII.push({ registro, categoria_equipamento: categoriaKey as any, tipo_suprimento: tipoSuprimento, valor_total_linha: valorTotalLinha, total_litros_linha: totalLitrosLinha, preco_litro_linha: precoLitroLinha, memoria_calculo: memoriaCalculo });
            });
        }
    });
    return grupos;
  ...
  const omsOrdenadas = useMemo(() => {
    ...
    return oms.sort((a, b) => {
        const aPriority = getOMPriority(a, nomeRM);
        const bPriority = getOMPriority(b, nomeRM);
  ...
  const calcularTotaisClasseIII = (linhas: LinhaClasseIII[]) => {
    let dieselLitros = 0, gasolinaLitros = 0, valorDiesel = 0, valorGasolina = 0;
    linhas.forEach(linha => {
      if (linha.tipo_suprimento === 'COMBUSTIVEL_DIESEL') { dieselLitros += Number(linha.total_litros_linha || 0); valorDiesel += Number(linha.valor_total_linha || 0); }
      if (linha.tipo_suprimento === 'COMBUSTIVEL_GASOLINA') { gasolinaLitros += Number(linha.total_litros_linha || 0); valorGasolina += Number(linha.valor_total_linha || 0); }
    });
    return { dieselLitros, gasolinaLitros, valorDiesel, valorGasolina, valorTotalCombustivel: valorDiesel + valorGasolina };
  };
  ...
  const calcularTotaisPorOM = useCallback((grupo: GrupoOM, nomeOM: string) => {
    ...
    const totalLubrificante = grupo.linhasClasseIII.filter(l => l.tipo_suprimento === 'LUBRIFICANTE').reduce((acc, linha) => acc + linha.valor_total_linha, 0);
    const total_33_90_30 = totalQS + totalQR + totalClasseII_ND30 + totalClasseV_ND30 + totalClasseVI_ND30 + totalClasseVII_ND30 + totalClasseVIII_ND30 + totalClasseIX_ND30 + totalLubrificante; 
    ...
    const { dieselLitros, gasolinaLitros, valorTotalCombustivel, valorDiesel, valorGasolina } = isRMFornecedora ? calcularTotaisClasseIII(grupo.linhasClasseIII) : { dieselLitros: 0, gasolinaLitros: 0, valorTotalCombustivel: 0, valorDiesel: 0, valorGasolina: 0 };
    return { total_33_90_30, total_33_90_39, total_parte_azul, total_combustivel: valorTotalCombustivel, total_gnd3: total_parte_azul + valorTotalCombustivel, totalDieselLitros: dieselLitros, totalGasolinaLitros: gasolinaLitros, valorDiesel, valorGasolina };
  }, [nomeRM]);
  
  const gruposOperacionaisPorOM = useMemo(() => {
    const grupos: Record<string, GrupoOMOperacional> = {};
    const initializeGroup = (name: string) => { if (!grupos[name]) grupos[name] = { diarias: [], verbaOperacional: [], suprimentoFundos: [], passagens: [], concessionarias: [], materialConsumo: [], complementoAlimentacao: [], servicosTerceiros: [] }; };
    const getCreditRecipientOM = (record: any, isDetentoraBased: boolean) => isDetentoraBased ? (record.om_detentora || record.organizacao) : record.organizacao;
    registrosDiaria.forEach(r => { const om = getCreditRecipientOM(r, false); initializeGroup(om); grupos[om].diarias.push(r); });
    registrosVerbaOperacional.forEach(r => { const om = getCreditRecipientOM(r, true); initializeGroup(om); grupos[om].verbaOperacional.push(r); });
    registrosSuprimentoFundos.forEach(r => { const om = getCreditRecipientOM(r, false); initializeGroup(om); grupos[om].suprimentoFundos.push(r); });
    registrosPassagem.forEach(r => { const om = getCreditRecipientOM(r, true); initializeGroup(om); grupos[om].passagens.push(r); });
    registrosConcessionaria.forEach(r => { const om = getCreditRecipientOM(r, true); initializeGroup(om); grupos[om].concessionarias.push(r); });
    registrosMaterialConsumo.forEach(r => { const om = getCreditRecipientOM(r, true); initializeGroup(om); grupos[om].materialConsumo.push(r); });
    registrosComplementoAlimentacao.forEach(r => {
        if (r.categoria_complemento === 'genero') {
            const omQR = r.organizacao; initializeGroup(omQR); grupos[omQR].complementoAlimentacao.push({ registro: r, subType: 'QR' });
            const omQS = r.om_qs || r.organizacao; initializeGroup(omQS); grupos[omQS].complementoAlimentacao.push({ registro: r, subType: 'QS' });
        } else { const om = getCreditRecipientOM(r, true); initializeGroup(om); grupos[om].complementoAlimentacao.push({ registro: r }); }
    });
    registrosServicosTerceiros.forEach(r => { const om = getCreditRecipientOM(r, true); initializeGroup(om); grupos[om].servicosTerceiros.push(r); });
    return grupos;
  ...
  const omsHorasVooOrdenadas = useMemo(() => Object.keys(gruposHorasVooPorOM).sort((a, b) => { const aP = getOMPriority(a, nomeRM), bP = getOMPriority(b, nomeRM); return aP !== bP ? aP - bP : a.localeCompare(b); }), [gruposHorasVooPorOM, nomeRM]);
  const omsOperacionaisOrdenadas = useMemo(() => Object.keys(gruposOperacionaisPorOM).sort((a, b) => { const aP = getOMPriority(a, nomeRM), bP = getOMPriority(b, nomeRM); return aP !== bP ? aP - bP : a.localeCompare(b); }), [gruposOperacionaisPorOM, nomeRM]);
    ...
    switch (selectedReport) {
      ...
      case 'dor':
        ...
        const dorSelector = registrosDOR.length > 1 ? (
          <div className="flex items-center gap-3"><span className="text-sm font-bold whitespace-nowrap">Selecionar Documento:</span><Select value={selectedDorId || ''} onValueChange={setSelectedDorId}><SelectTrigger className="w-[300px] bg-white"><SelectValue placeholder="Escolha o DOR" /></SelectTrigger><SelectContent>{registrosDOR.map(dor => (<SelectItem key={dor.id} value={dor.id}>DOR Nr {dor.numero_dor || 'S/N'} - {new Date(dor.created_at).toLocaleDateString()}</SelectItem>))}</SelectContent></Select></div>
        ) : null;
        return <PTrabDORReport ptrabData={ptrabData} dorData={selectedDor} selector={dorSelector} />;
      default: return null;
    }
  };
