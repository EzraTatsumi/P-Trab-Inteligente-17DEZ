 ...
 * Se não houver customizado, usa o campo 'detalhamento' (preenchido pelo formulário).
 */
export const generateConcessionariaMemoriaCalculada = (
    ...
    // Prioriza o campo 'detalhamento', que contém a memória gerada pelo formulário
    if (registro.detalhamento && registro.detalhamento.trim().length > 0) {
        return registro.detalhamento;
    }
    
    // Fallback: Gera a memória consolidada (se for um registro consolidado)
    // Nota: Como o relatório operacional consolida, esta função é chamada para registros individuais.
    // Se for um registro individual sem detalhamento, usamos a função de utilidade individual.
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('concessionaria_registros').select('*, diretriz_id, detalhamento, detalhamento_customizado').eq('p_trab_id', ptrabId), // NOVO: Busca de Concessionária (incluindo detalhamento)
      ]);
