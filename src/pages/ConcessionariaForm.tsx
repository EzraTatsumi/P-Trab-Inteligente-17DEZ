...
interface CalculatedConcessionaria extends TablesInsert<'concessionaria_registros'> {
    ...
    selected_diretriz: ConcessionariaDiretrizSelection;
}
...
interface ConcessionariaFormState {
    ...
    // Dados das Diretrizes Selecionadas (Lista de ConcessionariaDiretrizSelection)
    selected_diretrizes: ConcessionariaDiretrizSelection[];
}
...
// Função para calcular o valor total de uma diretriz
const calculateDiretrizTotal = (diretriz: ConcessionariaDiretrizSelection): number => {
    return diretriz.quantidade_solicitada * diretriz.custo_unitario; 
};
...
const compareFormData = (data1: ConcessionariaFormState, data2: ConcessionariaFormState) => {
    ...
    // Comparar detalhes das diretrizes (IDs e quantidades solicitadas)
    const selections1 = data1.selected_diretrizes.map(d => `${d.id}-${d.quantidade_solicitada}`).sort().join('|');
    const selections2 = data2.selected_diretrizes.map(d => `${d.id}-${d.quantidade_solicitada}`).sort().join('|');
...
const ConcessionariaForm = () => {
    ...
    const [pendingRegistros, setPendingRegistros] = useState<CalculatedConcessionaria[]>([]);
    ...
    // Registros de Concessionária usam a tabela 'concessionaria_registros'
    ...
    const insertMutation = useMutation({
        mutationFn: async (newRecords: CalculatedConcessionaria[]) => {
            ...
            const recordsToInsert: TablesInsert<'concessionaria_registros'>[] = newRecords.map(r => {
                ...
                // Detalhamento: Nome da Concessionária - Consumo/Pessoa/Dia (Unidade/dia)
                const detalhamento = `${diretriz.nome_concessionaria} - Consumo: ${diretriz.consumo_pessoa_dia} (${diretriz.unidade_custo}/dia)`;
        ...
        onSuccess: () => {
            toast.success(`Sucesso! ${pendingRegistros.length} registro(s) de Concessionária adicionado(s).`);
            setPendingRegistros([]);
    ...
    const calculos = useMemo(() => {
        if (!ptrabData || formData.selected_diretrizes.length === 0) {
            return {
                totalGeral: 0,
                totalND39: 0,
        ...
        try {
            ...
            formData.selected_diretrizes.forEach((diretriz) => {
                const totalDiretriz = calculateDiretrizTotal(diretriz);
                ...
                tempGroup.records.push({
                    ...
                    detalhamento: `${diretriz.nome_concessionaria} - Consumo: ${diretriz.consumo_pessoa_dia} (${diretriz.unidade_custo}/dia)`, 
            ...
            tempGroup.totalND39 = totalGeral;
            
            const memoria = generateConsolidatedConcessionariaMemoriaCalculo(tempGroup);
            
            return {
                totalGeral,
                totalND39: totalGeral,
                memoria,
            };
        } catch (e) {
    ...
    const handleEdit = (group: ConsolidatedConcessionaria) => {
        ...
        const diretrizesFromRecords: ConcessionariaDiretrizSelection[] = group.records.map(registro => ({
            id: registro.diretriz_id, 
            categoria: registro.categoria as CategoriaConcessionaria,
            nome_concessionaria: registro.detalhamento?.split(' - ')[0] || 'Diretriz Desconhecida',
            consumo_pessoa_dia: Number(registro.consumo_pessoa_dia || 0),
            custo_unitario: Number(registro.valor_unitario || 0),
            unidade_custo: registro.detalhamento?.match(/\((.*?)\/dia\)/)?.[1] || 'unidade',
            // Recalcula a quantidade solicitada a partir do valor total e unitário
            quantidade_solicitada: Number(registro.valor_unitario || 0) > 0 
                ? Number(registro.valor_total || 0) / Number(registro.valor_unitario || 1)
                : 0,
        }));
        ...
        const newPendingItems: CalculatedConcessionaria[] = group.records.map(registro => {
            const diretriz: ConcessionariaDiretrizSelection = {
                id: registro.diretriz_id, 
                categoria: registro.categoria as CategoriaConcessionaria,
                nome_concessionaria: registro.detalhamento?.split(' - ')[0] || 'Diretriz Desconhecida',
                consumo_pessoa_dia: Number(registro.consumo_pessoa_dia || 0),
    ...
    const handleStageCalculation = (e: React.FormEvent) => {
        ...
        try {
            ...
            const totalSolicitado = formData.selected_diretrizes.reduce((sum, d) => sum + d.quantidade_solicitada, 0);
            if (totalSolicitado <= 0) {
                throw new Error("A quantidade total de unidades solicitadas deve ser maior que zero.");
            }
            ...
            const newPendingItems: CalculatedConcessionaria[] = formData.selected_diretrizes.map(diretriz => {
                ...
                const tempRegistro: ConcessionariaRegistro = {
                ...
                let memoria = generateConcessionariaMemoriaCalculo(tempRegistro);
            ...
            if (editingId) {
                // MODO EDIÇÃO: Preserva a memória customizada do primeiro registro do grupo original
    ...
    const handleDiretrizSelected = (diretrizes: ConcessionariaDiretrizSelection[]) => {
    ...
    // --- Lógica de Edição de Quantidade de Diretriz no Formulário Principal ---
    const handleDiretrizQuantityChange = (diretrizId: string, quantity: number) => {
        if (quantity < 0) return;
        
        setFormData(prev => {
            const newSelections = prev.selected_diretrizes.map(d => 
                d.id === diretrizId ? { ...d, quantidade_solicitada: quantity } : d
            );
            
            return {
                ...prev,
                selected_diretrizes: newSelections,
            };
        });
    };
    ...
    const isSolicitationDataReady = formData.dias_operacao > 0 &&
                                    ...
                                    formData.selected_diretrizes.every(d => d.quantidade_solicitada > 0); 

    const isCalculationReady = isBaseFormReady && isSolicitationDataReady;
    
    // Lógica para a Seção 3
    const itemsToDisplay = editingId ? pendingRegistros : pendingRegistros;
            ...
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Solicitação de recursos para pagamento de contas de concessionárias (ND 33.90.39).
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                        ...
                                        <Card className="mt-4 rounded-lg p-4 bg-background">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                >
                                                    ...
                                                    {formData.selected_diretrizes.length > 0 ? `Alterar/Adicionar Diretrizes (${formData.selected_diretrizes.length} selecionadas)` : "Selecionar Diretrizes de Concessionária *"}
                                                </Button>
                                                ...
                                                {formData.selected_diretrizes.length > 0 && (
                                                    <div className="border p-3 rounded-md space-y-2">
                                                        <Table>
                                                            <TableHeader>
                                                                <TableRow>
                                                                    <TableHead className="w-[100px] text-center">Qtd Solicitada</TableHead>
                                                                        ...
                                                                        <TableRow key={diretriz.id}>
                                                                            <TableCell className="w-[100px]">
                                                                                <div className="flex items-center justify-center gap-1">
                                                                                    <Input
                                                                                        ...
                                                                                        min={0} 
                                                                                        placeholder="0.00"
                                                                                        value={diretriz.quantidade_solicitada === 0 ? "" : diretriz.quantidade_solicitada}
                                                                                        onChange={(e) => handleDiretrizQuantityChange(diretriz.id, parseFloat(e.target.value) || 0)}
                                                                                        onWheel={(e) => e.currentTarget.blur()} 
                                                                                        onKeyDown={(e) => { 
                                                                                        ...
                                                                                        disabled={!isPTrabEditable || isSaving}
                                                                                    />
                                                                                    <span className="text-xs text-muted-foreground">{diretriz.unidade_custo}</span>
                                                                                </div>
                                                                            </TableCell>
                                                                            <TableCell>
                                                                            ...
                                                                            <TableCell className="text-xs">
                                                                                <div className="flex flex-col">
                                                                                    <span className="font-medium">{diretriz.categoria}</span>
                                                                                </div>
                                                                            </TableCell>
                                ...
                                <section className="space-y-4 border-b pb-6">
                                    ...
                                    <div className="space-y-4">
                                        {itemsToDisplay.map((item) => {
                                            ...
                                            const diretriz = item.selected_diretriz;
                                            ...
                                            const totalUnidades = diretriz.quantidade_solicitada;
                                            const unidadeText = diretriz.unidade_custo;
                                                    ...
                                                    <CardContent className="p-4">
                                                        ...
                                                        <div className={cn("flex justify-between items-center pb-2 mb-2", "border-b border-secondary/30")}>
                                                            <h4 className="font-bold text-base text-foreground flex items-center gap-2">
                                                                {diretriz.categoria === 'Água/Esgoto' ? <Droplet className="h-4 w-4 text-blue-500" /> : <Zap className="h-4 w-4 text-yellow-600" />}
                                                                {diretriz.nome_concessionaria}
                                                            </h4>
                                                        ...
                                                        <div className="grid grid-cols-2 gap-4 text-xs pt-1">
                                                            <div className="space-y-1">
                                                                ...
                                                                <p className="font-medium">Qtd Solicitada:</p>
                                                            </div>
                                                            <div className="text-right space-y-1">
                                                                ...
                                                                <p className="font-medium">{totalUnidades.toFixed(2)} {unidadeText}</p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="flex justify-between text-xs">
                                                            <span className="text-muted-foreground">ND 33.90.39 (Concessionárias):</span>
                            ...
                            {consolidatedRegistros && consolidatedRegistros.length > 0 && (
                                <section className="space-y-4 border-b pb-6">
                                    ...
                                    {consolidatedRegistros.map((group) => {
                                        ...
                                        const categorias = group.records.map(r => r.categoria).join(', ');
                                                            ...
                                                            <div className="flex flex-col">
                                                                <div className="flex items-center gap-2">
                                                                    <h4 className="font-semibold text-base text-foreground">
                                                                        Concessionárias ({categorias})
                                                                    </h4>
                                                                </div>
                                                                <p className="text-xs text-muted-foreground">
                                                                    {group.records.length} diretriz(es) | Período: {diasOperacaoConsolidado} {diasText} | Efetivo: {efetivoConsolidado} {efetivoText}
                                                                </p>
                                                            </div>
                                                            <div className="flex items-center gap-2">
                                                                <span className="font-extrabold text-xl text-foreground">
                ...
                <ConcessionariaDiretrizSelectorDialog
                    ...
                    efetivo={formData.efetivo}
                    diasOperacao={formData.dias_operacao}
                />
            </div>
        </div>
    );
