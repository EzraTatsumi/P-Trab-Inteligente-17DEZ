...
// Função auxiliar para converter string de moeda (R$ X.XXX,XX) para número
const parseCurrency = (value: string): number => {
    // Remove R$, pontos de milhar e substitui vírgula decimal por ponto
    const cleanedValue = value.replace(/R\$\s*/g, '').replace(/\./g, '').replace(/,/g, '.').trim();
    return parseFloat(cleanedValue) || 0;
};

export const defaultClasseIXConfig: DiretrizClasseIIForm[] = [
  // Vtr Administrativa
