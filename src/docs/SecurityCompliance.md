...
- **Proteção de Tabelas:** Todas as tabelas sensíveis (`p_trab`, `classe_i_registros`, `organizacoes_militares`, etc.) têm RLS habilitado.
- **Regras de Propriedade:** Políticas garantem que `auth.uid()` (o ID do usuário logado) deve corresponder ao `user_id` do registro.

### Criptografia de Exportação
...
- **Cálculos Padronizados:** As fórmulas de Classe I, II e III são implementadas conforme as normas, minimizando erros humanos.
